   subroutine gf_HI_fullU(GF, Tail, e0f, Ur, umn, ujmn, zmsb, nlm, Iwmax, &
                          nmom, ns, atocc, atmag, temp, verbosity)
      ...
      ! ujmn /input/ - 2-index Coulomb interation U(m, m', m, m')-U(m, m', m',
      ! m)
      ...
      REAL(KIND=8), INTENT(out)    :: atocc(2*nlm), atmag(nlm)
      TYPE(h1occup), DIMENSION(0:nlm*ns) :: N_occ
      ...
      integer                            :: i, j, m, m1, is, is1, iom, ls, iii
      ...
      do i = 0, nstate - 1
         ...
         E_A(i) = dot_product__(ener, occ)
         docc = matmul(occ, ummss)
         E_A(i) = E_A(i) + dot_product__(docc, occ)/2.d0
      enddo
      ...
      DO i = 0, nstate - 1
         IF (ABS(ge - E_A(i)) < 1d-9) THEN
            ...
            do iii = 1, nlm
               atocc(iii) = atocc(iii) + occ(iii)
               atocc(iii + nlm) = atocc(iii + nlm) + occ(iii + nlm)
               atmag(iii) = atmag(iii) + (occ(iii) - occ(nlm + iii))
            enddo
      ...
      if (verbosity > 0) write (*, '(a, 50f12.5)') 'Atomic occupancy with 2-ind &
           &U :', atocc
      if (verbosity > 0) write (*, '(a, 50f13.7)') 'Ground state energy with &
           &2-ind U :', ge
      ...
      DO WHILE (iloc == 0)
         ...
         DO i = 0, nso
            ...
            CALL diagH(N_occ(i)%Hn, N_occ(i)%En, e0f, U, N_occ(i)%st_n, arr, &
                       N_occ(i)%narr, nso, nstate, i, N_occ(i)%n, verbosity)
            if (verbosity > 1) WRITE (*, '(a, I7, a, F14.7)') 'The lowest energy &
                 &for N = ', i, ' is ', N_occ(i)%En(1)
            if (verbosity > 1) write (*, '(a, I7, a, /)') 'i = ', i, ' done! <== '

         ENDDO

         ! If occupancy is constrained do not search for the true GS
         ...
         IF (Nat .NE. 0 .and. .NOT. N_occ(max(0, Nat - 1))%ifdiag) THEN
            N_occ(Nat - 1)%run = .TRUE.
         ELSEIF (Nat .NE. nso .and. .NOT. N_occ(min(nso, Nat + 1))%ifdiag) THEN
            N_occ(Nat + 1)%run = .TRUE.
         ELSE
            iloc = 1
         ENDIF
      ...
      IF (zerotemp) THEN
         ...
         DO i = 1, N_occ(Nat)%ndeg
            DO k = 1, N_occ(Nat)%n
               ...
               do iii = 1, nlm
                  atocc(iii) = atocc(iii) + occ(iii)*N_occ(Nat)%Hn(k, &
                                                                   i)*CONJG(N_occ(Nat)%Hn(k, i))
                  atmag(iii) = atmag(iii) + (occ(iii) - occ(nlm + &
                                                            iii))*N_occ(Nat)%Hn(k, i)*CONJG(N_occ(Nat)%Hn(k, i))
               enddo
               do iii = nlm + 1, 2*nlm
                  atocc(iii) = atocc(iii) + occ(iii)*N_occ(Nat)%Hn(k, &
                                                                   i)*CONJG(N_occ(Nat)%Hn(k, i))
         ...
         DO k = 0, nso
            ...
            DO i = 1, N_occ(k)%n
               ...
               DO l = 1, N_occ(k)%n
                  ...
                  do iii = 1, nlm
                     atocc(iii) = atocc(iii) + occ(iii)*N_occ(k)%Hn(l, &
                                                                    i)*CONJG(N_occ(k)%Hn(l, i))*Zterm
                     atmag(iii) = atmag(iii) + (occ(iii) - occ(nlm + &
                                                               iii))*N_occ(k)%Hn(l, i)*CONJG(N_occ(k)%Hn(l, &
                                                                                                         i))*Zterm
                  enddo
                  do iii = nlm + 1, 2*nlm
                     atocc(iii) = atocc(iii) + occ(iii)*N_occ(k)%Hn(l, &
      ...
      atmag = atmag/Z
      IF (zerotemp) THEN
         if (verbosity > 0) WRITE (*, '(a, i2, a, i5, a, f13.6)') 'The ground &
              &state has occupancy ', Nat, ', degeneracy ', N_occ(Nat)%ndeg, ' &
              &and energy ', N_occ(Nat)%En(1)
      ELSE
         if (verbosity > 0) WRITE (*, '(/, a, i2, a, f13.6)') 'The ground state &
              &has occupancy ', Nat, ' and energy ', Eground
         if (verbosity > 0) WRITE (*, '(a, i5, a)') 'Transitions from ', &
            N_occ(Nat)%ndeg, ' atomic states are included in GF'

         if (verbosity > 0) WRITE (*, '(a, f13.6)') 'Z = ', Z
      ENDIF
      if (verbosity > 0) write (*, '(a, 50f12.5)') 'Atomic occupancy  :', atocc
      if (verbosity > 0) write (*, '(a, 50f12.5)') 'Atomic mag. mom.  :', atmag
      ...
      DO i = 0, nso
         IF (.NOT. N_occ(i)%ifdiag .AND. N_occ(i)%run) THEN
            ...
            CALL diagH(N_occ(i)%Hn, N_occ(i)%En, e0f, U, N_occ(i)%st_n, arr, &
                       N_occ(i)%narr, nso, nstate, i, N_occ(i)%n, verbosity)
            if (verbosity > 1) WRITE (*, '(/, a, I7, a, F14.7)') 'The lowest &
                 &energy for N = ', i, ' is ', N_occ(i)%En(1)
         ENDIF
      ENDDO
      ...
      DO i = 0, nso
         ...
         IF (LEFT .OR. RIGHT) then
            CALL add_to_GF_N(GF, Tail, arr, nso, nmom, Nat, Iwmax, l - k + 1, &
                             N_occ(k:l), zmsb, Z, Eground, temp, zerotemp, LEFT, RIGHT)
         endif
      ENDDO
      IF (verbosity > 1) THEN
      ...
      DO i = 0, nso
         IF (N_occ(i)%ifdiag) DEALLOCATE (N_occ(i)%Hn, N_occ(i)%En, &
                                          N_occ(i)%st_n, N_occ(i)%narr)
      ENDDO
   ...
   subroutine diagH(H, E, e0f, U, st, arr, narr, nso, nstate, Nat, n, verbosity)
      ...
      INTEGER                           :: i, k, l, p, q, m, m1, m2, m3, m4, &
                                           INFO
      INTEGER                           :: ks, ls, ps, qs
      REAL(KIND=8)                    :: fsign
      ...
      ENDDO
      IF (k .NE. n) THEN
         WRITE (*, *) 'Error in diagH: array size is different from number of &
              &states'
         STOP
      ENDIF
      H = (0d0, 0d0)
      if (verbosity > 0) WRITE (*, '(a, i7)') 'Set up the Hamiltonian for N = ', &
         Nat
            ...
            DO m2 = 1, nso
               DO m3 = 1, nso
                  ...
                  DO i = 1, n
                     ...
                     H(narr(q), i) = H(narr(q), i) + 0.5d0*U(m, m1, m2, &
                                                             m3)*fsign
                  ENDDO
               ENDDO
            ENDDO
   ...
   subroutine vertex4ind(U, Uc, nlm, ns)
      ...
      REAL(KIND=8), INTENT(out), DIMENSION(nlm*ns, nlm*ns, nlm*ns, nlm*ns) &
         :: Uc
      ...
      DO is1 = 1, ns
         DO is2 = 1, ns
            !s1, s2, s1, s2
            Uc((is1 - 1)*nlm + 1:is1*nlm, (is2 - 1)*nlm + 1:is2*nlm, (is1 - 1)*nlm + &
               1:is1*nlm, (is2 - 1)*nlm + 1:is2*nlm) = U(:, :, :, :)
         ENDDO
      ENDDO
      RETURN
   end subroutine

   subroutine add_to_GF_N(GF, Tail, arr, nso, nmom, Nat, Iwmax, num, N_occ, &
                          zmsb, Z, Eground, temp, zerotemp, LEFT, RIGHT)

      ! Adds to the GF the contribution associated to transitions from the
      ! states with occupancy N
      ...
      IF (LEFT) THEN
         ...
         DO i = 1, N_occ(2)%ndeg
            DO k = 1, N_occ(2)%n
               DO m = 1, nso
                  ...
                  fsign = (-1d0)**ls
                  DO k1 = 1, N_occ(1)%n
                     anmat(m, k1, i) = anmat(m, k1, i) + CONJG(N_occ(2)%Hn(k, &
                                                                           i))*N_occ(1)%Hn(N_occ(1)%narr(l), k1)*fsign
                  ENDDO
               ENDDO
            ENDDO
         ENDDO
         ...
         DO i = 1, N_occ(2)%ndeg ! sum over contributing ground states
            DO k = 1, N_occ(1)%n              ! sum over excited states
               ...
               ELSE
                  ecoff = EXP((Eground - N_occ(1)%En(k))/temp) + &
                          EXP((Eground - N_occ(2)%En(i))/temp)
               ENDIF
               DO ie = 1, Iwmax
                  zener = 1d0/(zmsb(ie) - N_occ(2)%En(i) + N_occ(1)%En(k))*ecoff
                  DO m = 1, nso
                     DO m1 = 1, nso
                        GF(m, m1, ie) = GF(m, m1, ie) + anmat(m, k, &
                                                              i)*CONJG(anmat(m1, k, i))*zener
                     ENDDO
                  ENDDO
               ENDDO
               ...
               do ie = 1, nmom   ! number of moments
                  do m = 1, nso
                     do m1 = 1, nso
                        Tail(ie, m, m1) = Tail(ie, m, m1) + ecoff*anmat(m, &
                                                                        k, i)*CONJG(anmat(m1, k, i))* &
      ...
      IF (RIGHT) THEN
         ...
         DO i = 1, N_occ(num1)%ndeg
            DO k = 1, N_occ(num1)%n
               DO m = 1, nso
                  occ = merge(1.d0, 0.d0, btest(N_occ(num1)%st_n(k), &
                                                arr(0:nso - 1)))
                  ...
                  fsign = (-1d0)**ls
                  DO k1 = 1, N_occ(num)%n
                     crmat(m, k1, i) = crmat(m, k1, i) + &
                                       CONJG(N_occ(num1)%Hn(k, &
                                                            i))*N_occ(num)%Hn(N_occ(num)%narr(l), k1)*fsign
                  ENDDO
               ENDDO
            ENDDO
         ENDDO
         ...
         DO i = 1, N_occ(num1)%ndeg
            ...
            DO k = N_occ(num)%ndeg + 1, N_occ(num)%n
               ...
               ELSE
                  ecoff = EXP((Eground - N_occ(num)%En(k))/temp) + &
                          EXP((Eground - N_occ(num1)%En(i))/temp)
               ENDIF
               DO ie = 1, Iwmax
                  zener = 1d0/(zmsb(ie) - N_occ(num)%En(k) + &
                               N_occ(num1)%En(i))*ecoff
                  DO m = 1, nso
                     DO m1 = 1, nso
                        GF(m, m1, ie) = GF(m, m1, ie) + CONJG(crmat(m, k, &
                                                                    i))*crmat(m1, k, i)*zener
                     ENDDO
                  ENDDO
               ENDDO
               ...
               do ie = 1, nmom   ! number of moments
                  do m = 1, nso
                     do m1 = 1, nso
                        Tail(ie, m, m1) = Tail(ie, m, m1) + ecoff* &
                                          CONJG(crmat(m, k, i))*crmat(m1, k, i)* &
