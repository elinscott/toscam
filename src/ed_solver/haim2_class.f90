...
contains
   ...
   subroutine new_HAIM2(AIM, sector, SPIN)
      ...
      IF (PRESENT(SPIN)) THEN
         ...
         CASE (1)
            ...
            if (.not. USE_TRANSPOSE_TRICK_MPI) then
               ALLOCATE (noffup(sector%chunk(iproc)), &
                         diagup(sector%chunk(iproc)))
            else
            ...
            CALL tab_HAIM2(AIM2up, sector, NO_STORAGE=.true.)
         ...
         CASE (2)
            ...
            if (.not. USE_TRANSPOSE_TRICK_MPI) then
               ALLOCATE (noffdo(sector%chunk(iproc)), &
                         diagdo(sector%chunk(iproc)))
            else
            ...
            CALL tab_HAIM2(AIM2do, sector, NO_STORAGE=.true.)
      ...
      ELSE
         ...
         if (.not. ON_FLY) ALLOCATE (noffsz(sector%chunk(iproc)), &
                                     diagsz(sector%chunk(iproc)))

         ! HAMILTONIAN DEPENDANT ARRAYS
         CALL new_AIM2(AIM2sz, AIM)
         if (.not. ON_FLY) CALL tab_HAIM2(AIM2sz, sector, NO_STORAGE=.true.)
   ...
   subroutine delete_HAIM2(SPIN)
      ...
      use aim2_class, only: delete_aim2
   ...
   subroutine tab_HAIM2(AIM2, sector, NO_STORAGE)
      ...
      LOGICAL                  :: compute_noffdiag_only ! true if we only
      ! compute noffdiag
      INTEGER                  :: istate, jstate, istateloc, istatemin, &
                                  istatemax, iorb, jorb
...
!$OMP          END PARALLEL
      ...
      if (iproc /= 1) then
         ...
         write (*, *) 'either set FLAG_MPI to > 0, or recompile with &
              &-DOPENMP_MPI'
         stop
      endif
#else
      ...
      if (OPEN_MP) then
         if (sector%istatemax(iproc) > 0) then
            ...
            if (verboseall) write (*, *) 'SPLITTING HILBERT SPACE ON OMP, DIM = &
                 &', sector%istatemax(iproc) - sector%istatemin(iproc) + 1
            call &
               openmp_split_array(sector%istatemax(iproc) - sector%istatemin(iproc) &
                                  + 1, imin_, imax_)
      ...
      go_for_omp = .false.
      if (OPEN_MP) go_for_omp = minval(imax_) > 0 .and. minval(imin_) > 0 .and. &
                                ALL(sector%istatemax > 0)

!$OMP       PARALLEL IF(go_for_omp) PRIVATE(TID, istatemin_, istatemin0, &
!$OMP            istatemax0, istate, istateloc, jstate, iorb, jorb, &
!$OMP            noffdiag_save, ket_in, ket_out, noffdiagi)
      ...
      if (OPEN_MP .and. go_for_omp) then
...
#endif
                  if (noffdiagi < 0) stop 'error in starting openmp, noffdiagi &
                       &< 0'
               else
                  noffdiagi = 0
               endif
         ...
         DO istate = istatemin0, istatemax0
            ...
            IF (.NOT. compute_noffdiag_only) THEN
               ! IMPURITY QUADRATIC
               DO iorb = 1, IMPnorbs
                  IF (is_occupied(IMPiorb(iorb), ket_in)) diag(istateloc) = &
                     diag(istateloc) + Ec%rc%mat(iorb, iorb)
               ENDDO
               ! BATH
               DO iorb = 1, BATHnorbs
                  IF (is_occupied(BATHiorb(iorb), ket_in)) diag(istateloc) = &
                     diag(istateloc) + Eb%rc%mat(iorb, iorb)
               ENDDO
               if (do_quench == 1) then
                  if (AIM2%spin == 0) then !Sz
                     DO iorb = 1, IMPnorbs
                        IF (is_occupied(IMPiorb(iorb), ket_in)) diag(istateloc) &
                           = diag(istateloc) + quench_mag
                     ENDDO
                  endif
                        ...
                        IF (.NOT. ket_out%is_nil) THEN
                           ...
                           IF (USE_CC .or. jstate < istate) THEN
                              ...
                              IF (.NOT. compute_noffdiag_only) THEN
                                 ...
                                 offdiag(noffdiagi) = Ec%rc%mat(iorb, jorb)* &
                                                      ket_out%fermion_sign
                              ENDIF
                           ENDIF
                        endif
                     ENDIF
               ...
               DO jorb = 1, BATHnorbs
                  IF (iorb /= jorb .AND. Eb%rc%MASK%mat(iorb, jorb)) THEN
                     if (abs(Eb%rc%mat(iorb, jorb)) > cutoff_hamilt_param) then
                        ...
                        CALL hop(ket_out, BATHiorb(iorb), BATHiorb(jorb), &
                                 ket_in)
                        ...
                        IF (.NOT. ket_out%is_nil) THEN
                           ...
                           IF (USE_CC .or. jstate < istate) THEN
                              ...
                              IF (.NOT. compute_noffdiag_only) THEN
                                 rankoff(noffdiagi) = jstate
                                 offdiag(noffdiagi) = Eb%rc%mat(iorb, jorb)* &
                                                      ket_out%fermion_sign
                              ENDIF
                           ENDIF
                        ENDIF
                     ...
                     if (abs(Vbc%rc%mat(iorb, jorb)) > cutoff_hamilt_param) then
                        ...
                        IF (.NOT. ket_out%is_nil) THEN
                           ...
                           IF (USE_CC .or. jstate < istate) THEN
                              ...
                              IF (.NOT. compute_noffdiag_only) THEN
                                 rankoff(noffdiagi) = jstate
                                 offdiag(noffdiagi) = Vbc%rc%mat(iorb, jorb)* &
                                                      ket_out%fermion_sign
                              ENDIF
                           ENDIF
                        ENDIF
                        ...
                        IF (.NOT. ket_out%is_nil) THEN
                           ...
                           IF (USE_CC .or. jstate < istate) THEN
                              ...
                              IF (.NOT. compute_noffdiag_only) THEN
                                 rankoff(noffdiagi) = jstate
                                 offdiag(noffdiagi) = conj(Vbc%rc%mat(iorb, &
                                                                      jorb))*ket_out%fermion_sign
                              ENDIF
                           ENDIF
                        ENDIF
                     endif
            ...
            if (istateloc < 0 .or. noffdiagi < 0 .or. noffdiag_save < 0) stop &
               'error in tabulate HAIM, negative noffdiag'
...
35       continue
         ...
         write (log_unit, *) 'IN SECTOR THERE ARE ... OFF DIAG ELEMENTS : ', &
            noffdiag

         if (.not. compute_noffdiag_only .and. check_sum /= noffdiag) stop &
            'error check_sum on noff not valid'
         if (noffdiag < 0) then
            write (*, *) ' number of OFF_DIAG terms of the hamiltonian matrix &
                 &are : ', noffdiag
            write (*, *) ' number of diagonal terms : ', size(noff)
            do i = 1, size(noff), 100000
               noffdiag = long_sum(noff(1:min(size(noff), i)))
               write (*, *) 'partial sum of off-diag terms : ', i, noffdiag, &
                  long_sum(noff(1:min(size(noff), i)))
            enddo
         ...
         CALL timer_fortran(start_tabH, "# TABULATION OF THE HAMILTONIAN IN &
      ...
      contains
         ...
         subroutine check_openmp()
            ...
            use globalvar_ed_solver, only: istati, open_mp

            implicit none

            if (istate > size(sector%state)) then
               write (*, *) 'running open mp ?     : ', OPEN_MP
               write (*, *) 'size of hilbert space : ', size(sector%state), &
                  sector%dimen
