   subroutine init_solver(FILEIN, AIM, OLDGSFILE)
      ...
      use namelistmod, only: look_for_command_line_argument, &
         ...
         putel_in_namelist
      use globalvar_ed_solver, only: istati, list_sectors, nsec, nsec0, &
         SCAN_FULL_NUP_NDN
      use genvar, only: log_unit, no_mpi, rank, size2
      use eigen_sector_class, only: add_eigensector, copy_eigensector, &
      ...
      INTEGER                :: isec, sz2, npart, nup, ndo, isector, ff, &
                                nsec_, k, i, j, fillup
      ...
      write (log_unit, *) 'initialize SOLVER, number of sites is : ', &
         AIM%bath%Nb + AIM%Nc
      ...
      call namelist_init(nm, 400, name_of_namelist='ED_SOLVER partial input &
           &related to solver')
      ...
      IF (AIM%bath%SUPER) THEN ! READ LIST OF Sz-SECTORS
         ...
         call putel_in_namelist(nm, list_sectors, 'list_sectors', list0, 'list &
              &of Sz sectors')
         ...
         DO isec = 1, nsec_
            ...
            write (log_unit, *) &
               '------------------------------------------------------------'
            write (log_unit, *) 'build up new Sz sector, isec, nsec : ', isec, &
               nsec_
            ...
            ALLOCATE (sec%sz)
            CALL new_fermion_sector(sec%sz, npart, AIM%norbs, SZ=.true.)
      ...
      ELSE ! READ LIST OF (nup, ndo)-SECTORS !
         ...
         if (nsec0 < 0) then
            write (*, *) ' nup/ndn sectors and you want a negative number of &
                 &particles?'
            write (*, *) ' sec0 is      : ', nsec0
         ...
         if (.not. SCAN_FULL_NUP_NDN) then
            ...
            call putel_in_namelist(nm, list_sectors, 'list_sectors', list0, &
                                   'list of sectors nup, ndo')
            call look_for_namelist_in_file(nm, FILEIN)
            call look_for_command_line_argument(nm)
            write (log_unit, *) ' LIST OF SECTOR TO CONSIDER ARE : ', list0
            ...
            do i = nsec0, nsec0 + nsec_ - 1
               do j = nsec0, nsec0 + nsec_ - 1
                  if ((i /= fillup .or. j /= fillup) .or. &
                      .not. discard_full_state) then
                     k = k + 1
                     list0(k) = i
                     k = k + 1
                     list0(k) = j
            ...
            if (k /= size(list0)) then
               ...
               stop 'error in solver.f90 in ed solver, size of list0 not ok, &
                    &critical'
            endif
            call putel_in_namelist(nm, list_sectors, 'list_sectors', list0, &
                                   'list of sectors nup, ndo')
         ...
         DO isec = 1, size(list_sectors)/2
            ...
            ALLOCATE (sec%updo)
            CALL new_fermion_sector2(sec%updo, nup, ndo, AIM%Ns, NAMBU=.false.)
            CALL new_eigensector(es, sec)
            CALL add_eigensector(es, sector2diagH)
         ENDDO
      ENDIF
      ...
      IF (PRESENT(OLDGSFILE)) THEN
         ...
         ! AT THIS POINT WE HAVE THE LIST OF EIGENSECTORS WITH DUMMY EMPTY
         ! SECTOR
         ...
         DO isector = 1, oldGS%nsector
            ...
            IF (ASSOCIATED(es%sector%updo)) THEN
               IF (es%sector%updo%up%norbs /= AIM%Ns .OR. &
                    es%sector%updo%down%norbs /= AIM%Ns) STOP "ERROR IN &
                    &init_solver: INCONSISTENT INPUT SECTOR!"
            ELSE IF (ASSOCIATED(es%sector%sz)) THEN
               IF (es%sector%sz%norbs /= AIM%norbs) STOP "ERROR IN init_solver: &
                    &INCONSISTENT INPUT SECTOR!"
            ENDIF
            ! THE INPUT GS ISNT IN sector2diagH: WE NEED TO CREATE & APPEND
            ! THIS SECTOR TO sector2diagH!
            IF (rank_sector_in_list(es%sector, sector2diagH) == 0) CALL &
               add_eigensector(es, sector2diagH)
