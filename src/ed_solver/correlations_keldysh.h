   subroutine keldysh()
      ...
      use globalvar_ed_solver, only: beta_ed, do_keldysh_gbigger, do_keldysh, &
      ...
      use eigen_sector_class, only: add_eigensector, copy_eigensectorlist, &
         ...
         gsenergy, new_eigensector, partition
      use apply_ns, only: apply_id, n_sector
      ...
      use linalg, only: dexpc, scalprod
      use solver, only: solve_aim
      use correlations, only: GKret

      implicit none

      integer                                 :: lesser_bigger, ispin, ii, jj, &
                                                 i, j, Nc, isector, ieigen, &
                                                 isector2, ieigen2
      ...
      write (*, *) 'STARTING KELDYSH AT FINITE TEMPERATURE WITH [x] ORBITALS : &
           &', G(1)%N
      Nc = G(1)%N
      ...
      if (allocated(green_lesser)) deallocate (green_lesser, green_bigger, &
                                               green_ret, green_ad)
      ...
      if (keldysh_pert_ground_sector) then
         ...
         do isector2 = 1, phi(1)%nsector
            do ieigen2 = 1, phi(1)%es(isector2)%lowest%neigen
               ...
               goto 989
33             continue
               phi(1)%es(isector2)%lowest%eigen(ieigen2)%vec%rc = &
                  GS%es(isector)%lowest%eigen(ieigen)%vec%rc
               write (*, *) 'NEW EIGENVECTOR FOR SECTOR/EIGENVALUE/VAL : ', &
                  isector2, ieigen2, &
                  phi(1)%es(isector2)%lowest%eigen(ieigen2)%val
989            continue
            enddo
         enddo
      endif
      ...
      do i = 1, keldysh_n - 1
         ...
         CALL compute_greenAA(GKret(1), AIM, beta_ED, phi(i), N_sector, &
                              apply_Id, COMPUTE_DYN=.true., keldysh_level=1, GS_out= &
                              phi(i + 1))
      enddo
               ...
               do ii = 1, keldysh_n
                  ...
                  if (jj == quench_orb .or. quench_orb == -1) then
                     ...
                     DO isector = 1, tmp0%nsector
                        if (ispin == 1) then
                           CALL Cup_sector(Asec, pm(pmi), &
                                           tmp0%es(isector)%sector)
                        else
                           CALL Cdo_sector(Asec, pm(pmi), &
                                           tmp0%es(isector)%sector)
                        endif
                        ...
                        DO ieigen = 1, tmp0%es(isector)%lowest%neigen
                           if (ispin == 1) then
                              CALL apply_Cup(Apm_es, pm(pmi), ORBMASKvec, &
                                             tmp0%es(isector), ieigen)
                           else
                              CALL apply_Cdo(Apm_es, pm(pmi), ORBMASKvec, &
                                             tmp0%es(isector), ieigen)
                           endif
                        ENDDO
               ...
               if (jj == quench_orb .or. quench_orb == -1) then
                  ...
                  do ii = 1, keldysh_n
                     ...
                     do i = 1, ii - 1
                        ...
                        CALL compute_greenAA(GKret(ispin), AIM, beta_ED, &
                                             tmp(i), N_sector, apply_Id, COMPUTE_DYN=.true., &
               ...
               do j = i, keldysh_n
                  do ii = 1, Nc
                     do jj = 1, Nc
                        ...
                        if ((jj == quench_orb .and. ii == quench_orb) .or. &
                            quench_orb == -1) then
                           ...
                           if (abs(Zpart1 - Zpart2) > 1.d-5) then
                              write (*, *) 'ERROR partition functions not &
                                   &consistent !!'
                           ...
                           do isector = 1, psi(i, ii, ispin)%nsector
                              ntot = ntot + psi(i, ii, &
                                                ispin)%es(isector)%lowest%neigen
                           enddo

                           do isector = 1, psi(i, ii, ispin)%nsector
                              do ieigen = 1, psi(i, ii, &
                                                 ispin)%es(isector)%lowest%neigen

                                 do isector2 = 1, psi(j, jj, ispin)%nsector
                                    do ieigen2 = 1, psi(j, jj, &
                                                        ispin)%es(isector2)%lowest%neigen
                                 ...
                                 goto 990
31                               continue
                                 boltz = DEXPc(-beta_ED*(psi(i, ii, &
                                                             ispin)%es(isector)%lowest%eigen(ieigen)%val - E01)) &
                                         /Zpart1
                                 if (quench_cancel_statistics) boltz = 1.d0/ &
                                                                       dble(ntot)
                                 if (lesser_bigger == 1) then
                                    cscal = scalprod(psi(j, jj, &
                                                         ispin)%es(isector2)%lowest%eigen(ieigen2)%vec%rc, &
                                                     psi(i, ii, ispin)%es(isector)%lowest%eigen(ieigen)%vec%rc)
                                    green_lesser(i, j, ii, jj, ispin) = &
                                       green_lesser(i, j, ii, jj, ispin) + &
                                       imi*boltz*cscal
                                    if (i /= j) green_lesser(j, i, ii, jj, &
                                                             ispin) = -conjg(green_lesser(i, j, &
                                                                                          ii, jj, ispin))
                                 else
                                    cscal = scalprod(psi(i, ii, &
                                                         ispin)%es(isector)%lowest%eigen(ieigen)%vec%rc, &
                                                     psi(j, jj, ispin)%es(isector2)%lowest%eigen(ieigen2)%vec%rc)
                                    green_bigger(i, j, ii, jj, ispin) = &
                                       green_bigger(i, j, ii, jj, ispin) - &
                                       imi*boltz*cscal
                                    if (i /= j) green_bigger(j, i, ii, jj, &
                                                             ispin) = -conjg(green_bigger(i, j, &
                                                                                          ii, jj, ispin))
                                 endif
990                              continue
      ...
      if (maxval(abs(green_lesser(:, :, 1, 1, 1) - &
                     (-conjg(transpose(green_lesser(:, :, 1, 1, 1)))))) > 1.d-5) then
         write (*, *) 'ERROR G^ < not proper time symmetry'
         write (*, *) 'diff = : ', maxval(abs(green_lesser(:, :, 1, 1, 1) - &
                                              (-conjg(transpose(green_lesser(:, :, 1, 1, 1))))))
      endif
      if (maxval(abs(green_bigger(:, :, 1, 1, 1) - &
                     (-conjg(transpose(green_bigger(:, :, 1, 1, 1)))))) > 1.d-5) then
         write (*, *) 'ERROR G^ > not proper time symmetry'
         write (*, *) 'diff = ', maxval(abs(green_bigger(:, :, 1, 1, 1) - &
                                            (-conjg(transpose(green_bigger(:, :, 1, 1, 1))))))
      endif

      do ispin = 1, 2
         do ii = 1, Nc
            do jj = 1, Nc
               green_ret(:, :, ii, jj, ispin) = +(green_bigger(:, :, ii, jj, &
                                                               ispin) - green_lesser(:, :, ii, jj, ispin))
               green_ad(:, :, ii, jj, ispin) = -(green_bigger(:, :, ii, jj, &
                                                              ispin) - green_lesser(:, :, ii, jj, ispin))
               do i = 1, keldysh_n
                  do j = 1, keldysh_n
      ...
      do i = 1, keldysh_n
         do ii = 1, Nc
            do jj = 1, Nc
               do ispin = 1, 2
                  n_v_t_matrix(i, ii, jj, ispin) = -imi*green_lesser(i, i, ii, &
                                                                     jj, ispin)
               enddo
            enddo
         enddo
      ...
      do ispin = 1, 2
         open (unit=918181, file='n_time_matrix_'// &
               trim(adjustl(toString(ispin))), form='unformatted')
      ...
      do jj = 1, Nc
         do ispin = 1, 2
            !!call plotarray(time_mesh, real(green_lesser(1, :, jj, jj,
            ! ispin)), 'Real_glesser_0T_orb_' // trim(adjustl(toString(jj)))
            ! // '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, aimag(green_lesser(1, :, jj, jj,
            ! ispin)), 'Im_glesser_0T_orb_' // trim(adjustl(toString(jj))) //
            ! '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, real(green_bigger(:, 1, jj, jj,
            ! ispin)), 'Real_gbigger_T0_orb_' // trim(adjustl(toString(jj)))
            ! // '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, aimag(green_bigger(:, 1, jj, jj,
            ! ispin)), 'Im_gbigger_T0_orb_' // trim(adjustl(toString(jj))) //
            ! '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, real(green_lesser(:, 1, jj, jj,
            ! ispin)), 'Real_glesser_T0_orb_' // trim(adjustl(toString(jj)))
            ! // '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, aimag(green_lesser(:, 1, jj, jj,
            ! ispin)), 'Im_glesser_T0_orb_' // trim(adjustl(toString(jj))) //
            ! '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, real(green_bigger(1, :, jj, jj,
            ! ispin)), 'Real_gbigger_0T_orb_' // trim(adjustl(toString(jj)))
            ! // '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, aimag(green_bigger(1, :, jj, jj,
            ! ispin)), 'Im_gbigger_0T_orb_' // trim(adjustl(toString(jj))) //
            ! '_' // trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, real(green_ret(:, 1, jj, jj, ispin)),
            ! 'Real_gret_T0_orb_' // trim(adjustl(toString(jj))) // '_' //
            ! trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, aimag(green_ret(:, 1, jj, jj, ispin)),
            ! 'Im_gret_T0_orb_' // trim(adjustl(toString(jj))) // '_' //
            ! trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, real(green_ret(1, :, jj, jj, ispin)),
            ! 'Real_gret_0T_orb_' // trim(adjustl(toString(jj))) // '_' //
            ! trim(adjustl(toString(ispin))))
            !!call plotarray(time_mesh, aimag(green_ret(1, :, jj, jj, ispin)),
            ! 'Im_gret_0T_orb_' // trim(adjustl(toString(jj))) // '_' //
            ! trim(adjustl(toString(ispin))))
            do i = 1, keldysh_n
               n_versus_t(i, ispin) = -imi*green_lesser(i, i, jj, jj, ispin)
            enddo
            !!call plotarray(time_mesh, n_versus_t(:, ispin),
            ! 'n_versus_t_orb_' // trim(adjustl(toString(jj))) // '_' //
            ! trim(adjustl(toString(ispin))))
         enddo
         !!call plotarray(time_mesh, (n_versus_t(:, 1)-n_versus_t(:, 2))/2.d0,
         ! 'mag_versus_t_orb_' // trim(adjustl(toString(jj))))
      enddo
