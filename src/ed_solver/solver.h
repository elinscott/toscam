   subroutine get_GS()
      ...
      use genvar, only: log_unit, size2
      ...
      DO isector = rank_ + 1, GS%nsector, size2_
         if (GS%es(isector)%lowest%neigen > 0) then
            ...
            if (iterdmft == 1 .or. restarted .or. .not. track_sectors .or. (uup &
                                                                     /= -1000 .and. (uup <= nupmax + iwindow .and. ddn <= ndnmax + &
                                                                                iwindow .and. uup >= nupmin - iwindow .and. ddn >= &
                                                                       ndnmin - iwindow)) .or. (ssz /= -1000 .and. (ssz <= szmax + &
                                                                                        iwindow .and. ssz >= szmin - iwindow))) then

               if (.not. (USE_TRANSPOSE_TRICK_MPI .and. NOT_COMMENSURATE)) then

                  IF (Block_size == 0 .or. uup == 0 .or. ddn == 0 .or. uup == 1 &
                      .or. ddn == 1 .or. (uup == itot .and. ddn == itot) &
                      ) THEN
   ...
   subroutine get_eigen_energies()
      ...
      use ed_arpack, only: arpack_diago, ed_diago
      use genvar, only: huge_, log_unit, size2
      use globalvar_ed_solver, only: block_size, demax, &
         flag_dump_info_for_gamma_vertex, flag_mpi_greens, iterdmft, &
         iwindow, ndnmax, ndnmin, nupmax, nupmin, restarted, szmax, szmin, &
         track_sectors, use_transpose_trick_mpi, which_lanczos
      use h_class, only: delete_h, new_h
...
31       continue
      ...
      DO isector = rank_ + 1, GS%nsector, size2_
         ...
         if (GS%es(isector)%lowest%neigen > 0) &
            GS%es(isector)%lowest%eigen(:)%val = huge_

         goforit = iterdmft == 1 .or. restarted .or. .not. track_sectors
         if (sector_bound_file_present) goforit = .false.

         goforit = goforit .or. (uup /= -1000 .and. (uup <= nupmax + iwindow &
                                                     .and. ddn <= ndnmax + iwindow .and. uup >= nupmin - iwindow .and. &
                                                     ddn >= ndnmin - iwindow))
         goforit = goforit .or. (ssz /= -1000 .and. (ssz <= szmax + iwindow &
                                                     .and. ssz >= szmin - iwindow))
         ...
         if (goforit) then
            ...
            if (.not. (USE_TRANSPOSE_TRICK_MPI .and. NOT_COMMENSURATE)) then
               ...
               IF (Block_size == 0 .or. uup == 0 .or. ddn == 0 .or. uup == 1 &
                   .or. ddn == 1 .or. (uup == itot .and. ddn == itot)) THEN
                  ...
                  CASE ('NORMAL')
                     ...
                     !   write(*, *) '---start GPU Lanczos---'
                     ! CALL
                     ! Lanczos_fast_diagonalize_GPU(GS%es(isector)%lowest)
                  CASE ('FULL_ED')
               ...
               ELSE
                  ...
                  if (USE_TRANSPOSE_TRICK_MPI) stop 'error block lanczos and &
                       &lanczos vector split among nodes, stop - critical '

#ifdef _complex
                  write (log_unit, *) 'Cullum library not installed - to be &
                       &added in the future'
                  stop
#else
                  IF (Block_size < 0) THEN
               ...
               if (GS%es(isector)%lowest%neigen > 0) then
                  if (.not. FLAG_FULL_ED_GREEN) then
                     ...
                     write (log_unit, *) 'NupNdn File : ', &
                        sector_bound_file_present
   ...
   subroutine sync_eigen_energies()
      ...
      DO iii = 1, GS%nsector
         ...
         if (neigen > 0) then
            ...
            if (ii == rank) then
               do kk = 1, neigen
                  coef_lanc(1:dim_lanc(kk), kk) = &
                     GS%es(iii)%lowest%eigen(kk)%lanczos_vecp(1:dim_lanc(kk))
               enddo
            endif
            ...
            if (ii /= rank) then
               do jj = 1, neigen
                  CALL new_eigen(eigen, VAL(jj), tempvec, .true., RANK=jj, &
                                 no_vector=.true.)
                  eigen%lanczos_iter = dim_lanc(jj)
                  eigen%lanczos_vecp(1:eigen%lanczos_iter) = &
                     coef_lanc(1:dim_lanc(jj), jj)
                  eigen%rdist = 0.
                  ...
                  write (*, *) ' eigenvalues : ', &
                     GS%es(iii)%lowest%eigen(jj)%val
                  write (*, *) ' SECTOR dimension : ', &
                     GS%es(iii)%lowest%eigen(jj)%dim_space
               enddo
            endif
   ...
   subroutine get_info_sector()
      ...
      if (AIM%bath%SUPER) then
         ...
         if (USE_TRANSPOSE_TRICK_MPI) stop 'error sz basis and lanczos vector &
              &split among nodes, not implemented - critical'
      else
         ssz = -1000
