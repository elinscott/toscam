   ...
   if (MAXT > 1 .and. size2 > 1) write (*, *) 'WARNING : TRY TO RUN MPI AND OPEN-mP &
        &AT THE SAME TIME'
   if (OPEN_MP) then
   ...
   if (USE_TRANSPOSE_TRICK_MPI .and. .not. enable_mpi_dot) then
      stop 'ERROR USE_TRANSPOSE_TRICK_MPI needs enable_mpi_dot enabled, because&
           & the lanczos vector are split among the cpu'
   endif
   ...
   call init_frequency_module(rdelta_frequ_eta1, rdelta_frequ_w0, &
                              rdelta_frequ_T, rdelta_frequ_eta2_=rdelta_frequ_eta2)

   if (Niter_search_max_0 == 0) Niter_search_max_0 = Niter_search_max

   if (ncpt_approx /= 0) then
      PAIRING_IMP_TO_BATH = .false.
      write (*, *) 'WARNING turning off pairing imp to bath, not compatible with&
           & CPT approximation yet'
   endif

   do_quench_ = do_quench

   if (keldysh_pert_ground_sector) then
      write (logfile, *) 'WARNING : for keldysh, setting the flag keldysh_pert_g&
           &round_sector'
      write (logfile, *) '          this sets dEmax0 to a small value, in order &
           &to only get the ground state in each sector'
      write (logfile, *) '          and this also cancel the flag quench_cancel_&
           &statistics'
      dEmax0 = 0.00001
