...
c      normalize eps by the range of the X values
        ...
        if(abs(x(imx(npoint)-x(imx(1))))>1.d-4)then
         eps= eps/(  x( imx(npoint)) - x( imx(1)) )
        endif
 
        write(*,*) 'define eps...'
        write(*,*)  eps, npoint, size(imx), imx(npoint), imx(1)
...
c     handle condition for interpolation        
      ...
      write(*,*)  'define P expt...' 
      if(itp.eq.0) then   
          P=1.d0/(npoint*dexp(min(700.d0,h))+ 1.d0)  
      else  
          P=1.d0  
      endif  
