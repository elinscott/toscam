...
c  are invalid, control is immediately repassed to the calling program.
      ...
      if(k.le.0 .or. k.gt.5) then
       write(*,*) 'curfit error 1'
       go to 50
      endif
      ...
      if(iopt.lt.(-1) .or. iopt.gt.1) then
       write(*,*) 'curfit error 2'
       go to 50
      endif
      nmin = 2*k1
      if(m.lt.k1 .or. nest.lt.nmin) then
       write(*,*) 'curfit error 3'
       go to 50
      endif
      lwest = m*k1+nest*(7+3*k)
      if(lwrk.lt.lwest) then
         write(*,*) 'curfit error : work array too small'
         go to 50
      endif
      if(xb.gt.x(1) .or. xe.lt.x(m) .or. w(1).le.0.) then
        write(*,*) 'curfit error 4, wrong boundaries'
        write(*,*) 'xb,x(1),xe,x(m) : ',xb,x(1),xe,x(m)
        write(*,*) 'weight : ', w(1)
        write(*,*) 'm : ', m
        go to 50
      endif
      do 10 i=2,m
         if(x(i-1).ge.x(i) .or. w(i).le.0.) then 
          if(w(i).le.0.) then
           write(*,*) 'negative weight : ', w(i)
          endif
          if(x(i-1).ge.x(i))then
           write(*,*) 'order of points : ', x(i-1),x(i)
          endif
          write(*,*) 'curfit error 5'
          go to 50
         endif
  10  continue
      if(iopt.ge.0) go to 30
      if(n.lt.nmin .or. n.gt.nest) then
         write(*,*) 'curfit error 6'
         go to 50
      endif
      j = n
      do 20 i=1,k1
  ...
  30  if(s.lt.0.) go to 50
      if(s.eq.0. .and. nest.lt.(m+k1)) then
        write(*,*) 'curfit error 7'
        go to 50
      endif
      ier = 0
