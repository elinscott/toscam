...
c  the b-spline representation of s(x).
        ...
        i = n
        do 70 j=1,k1
          if(i<1) stop 'error fpcurf, i=0'
          if(i>size(t)) stop 'error outside boundary i'
          if(j<1) stop 'error fpcurf j=0'
          if(j>size(t)) stop 'error fpcurf outside boundary j'
          t(j) = xb
          t(i) = xe
          i = i-1
  70    continue
