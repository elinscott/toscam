...
! Input/output variables      
  ...
  REAL(KIND=8), INTENT(in) :: umn(nlm,nlm),ujmn(nlm,nlm),Ur(nlm,nlm,nlm,nlm)
...
!
  ...
  if (verbosity>0) write(*,'(a,50f12.5)') 'Atomic occupancy with 2-ind U :',atocc 
  if (verbosity>0) write(*,'(a,50f13.7)') 'Ground state energy with 2-ind U :',ge
  ...
  IF(zerotemp) THEN
     ...
     DO i=1,N_occ(Nat)%ndeg
        DO k=1,N_occ(Nat)%n
           ...
           do iii=1,nlm
            atocc(iii)=atocc(iii)+ occ(iii)*N_occ(Nat)%Hn(k,i)*CONJG(N_occ(Nat)%Hn(k,i))
            atmag(iii)=atmag(iii)+(occ(iii)-occ(nlm+iii))*N_occ(Nat)%Hn(k,i)*CONJG(N_occ(Nat)%Hn(k,i))
           enddo
           do iii=nlm+1,2*nlm
            atocc(iii)=atocc(iii)+occ(iii)*N_occ(Nat)%Hn(k,i)*CONJG(N_occ(Nat)%Hn(k,i))
           enddo
     ...
     DO k=0,nso
        ...
        DO i=1,N_occ(k)%n
           ...
           DO l=1,N_occ(k)%n
              ...
              do iii=1,nlm
              atocc(iii)=atocc(iii)+ occ(iii)*N_occ(k)%Hn(l,i)*CONJG(N_occ(k)%Hn(l,i))*Zterm
              atmag(iii)=atmag(iii)+(occ(iii)-occ(nlm+iii))*N_occ(k)%Hn(l,i)*CONJG(N_occ(k)%Hn(l,i))*Zterm
              enddo
              do iii=nlm+1,2*nlm
              atocc(iii)=atocc(iii)+ occ(iii)*N_occ(k)%Hn(l,i)*CONJG(N_occ(k)%Hn(l,i))*Zterm
              enddo
