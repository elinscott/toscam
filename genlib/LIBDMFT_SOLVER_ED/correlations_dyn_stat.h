  ...
  integer :: ii,jj,iii,jjj
      ...
      !---------!
       ...
       CALL compute_greenAA(G(1),AIM,beta_ED,GS,Cup_sector,apply_Cup,COMPUTE_DYN=.not.only_dens)
       CALL pad_green(G(2),G(1))

       if(FLAG_NCUP)then
         write(*,*) '--- COMPUTE PARTICLE/HOLE NDN_C_UP SPIN PART ---'
         CALL compute_greenAA(GN(1),AIM,beta_ED,GS,Cup_sector,apply_N_Cup,COMPUTE_DYN=.not.only_dens)
      ...
      if((.not.para_state.or.AIM%BATH%SUPER).and..not.FLAG_GUP_IS_GDN)then
         ...
         CALL compute_greenAA(G(2),AIM,beta_ED,GS,Cdo_sector,apply_Cdo,COMPUTE_DYN=.not.only_dens)
         if(FLAG_NCUP)then
          write(*,*) 'compute correlated hopping spin down'
          CALL compute_greenAA(GN(2),AIM,beta_ED,GS,Cdo_sector,apply_N_Cdo,COMPUTE_DYN=.not.only_dens)
         endif
      else
      ...
      if(FLAG_NCUP)then
         ...
         CALL compute_greenAB(GNF(1),GNF(2),GN(1),G(1),AIM,beta_ED,GS,Cup_sector,Cup_sector,apply_N_Cup,apply_Cup,COMPUTE_DYN=.not.only_dens)
      ...
      if(.not.only_dens)then
       IF(AIM%BATH%SUPER.AND.SUPER)THEN 
         ...
         CALL compute_greenAB(GF(1),GF(2),G(1),G(2),AIM,beta_ED,GS,Cup_sector,Cdo_sector,apply_Cup,apply_Cdo,COMPUTE_DYN=.not.only_dens)
         call write_array(  GF(1)%correl(1,1)%fctn(:,:,4) ,"GF(1) --",unit=98,short=.true.)
         call write_array(  GF(1)%correl(2,2)%fctn(:,:,4) ,"GF(2) ++",unit=98,short=.true.)
      ...
      if(.not.only_dens) CALL compute_self_energy(SNAMBU,GNAMBU%correl(2,1),AIM,retarded=.false.)
      write(*,*) '..... done .....'
  ...
  subroutine compute_retarded_functions
    ...
    !---------------------------------------------------------------------------!
    ! COMPUTE RETARDED GREEN S FUNCTION MATSUBARA+RETARDED BOSONIC CORRELATIONS !
    !---------------------------------------------------------------------------!
      ...
      IF(AIM%BATH%SUPER.AND.SUPER)THEN 
        ...
        CALL compute_greenAB(GFret(1),GFret(2),Gret(1),Gret(2),AIM,beta_ED,GS,Cup_sector,Cdo_sector,apply_Cup,apply_Cdo)
        CALL Nambu_green(GNAMBUret,G=Gret,GF=GFret)
      ELSE 
        CALL Nambu_green(GNAMBUret,G=Gret)
      ENDIF

      CALL compute_self_energy(SNAMBUret,GNAMBUret%correl(2,1),AIM,retarded=.true.)
   ...
   subroutine bosonic_equal_time
    ...
    !-------------------------------------------------------------------!
      ...
      write(*,*) ' ==== compute Sz ==== '
      if(always_compute_static_obs.or.compute_Sz) CALL compute_greenAA(Sz,AIM,beta_ED,GS,Sz_sector,apply_Sz,COMPUTE_DYN=F)
      if(dump_stat) call stats_func("APRES Sz")

      write(*,*) ' ==== compute Spm ==== '
      if(always_compute_static_obs.or.compute_Spm) CALL compute_greenAA(Spm,AIM,beta_ED,GS,S_sector,apply_S,COMPUTE_DYN=F)
      if(dump_stat) call stats_func("APRES Spm")

      write(*,*) ' ==== compute N ==== '
      if(always_compute_static_obs.or.compute_N) CALL compute_greenAA(N,AIM,beta_ED,GS,N_sector,apply_N,COMPUTE_DYN=F)
      if(dump_stat) call stats_func("APRES N")

      write(*,*) ' ==== P3 ==== '
      if(compute_P3) CALL compute_greenAA(P3,AIM,beta_ED,GS,P_sector,apply_P3,COMPUTE_DYN=F)
      if(dump_stat) call stats_func("APRES P3")

      write(*,*) ' ==== P4 ==== '
      if(compute_P4) CALL compute_greenAA(P4,AIM,beta_ED,GS,P_sector,apply_P4,COMPUTE_DYN=F)
   ...
   subroutine bosonic_dynamic
      ...
      IF(ANY(P3%compute))THEN
       ...
       CHI%fctn    = - (    P3%correl(1,1)%fctn +    P3%correl(2,2)%fctn - P3%correl(1,2)%fctn -    P3%correl(2,1)%fctn    ) * quarter**2
       CHIret%fctn = - ( P3ret%correl(1,1)%fctn + P3ret%correl(2,2)%fctn - P3ret%correl(1,2)%fctn - P3ret%correl(2,1)%fctn ) * quarter**2
