...
#include "integrate_class_tool.h"
...
T1 Integrate2c1(const mesh1D& omeps, const function1D<T1>& f0f1,
		const mesh1D& om2, const function<T2>& f2,
...
rT IntegrateInside2(const mesh1D& om0, const function<T0>& f0, double x0, double peak0,
		    const mesh1D& om1, const function<T1>& f1, double x1, double peak1,
		    const rT&)
{
...
rT IntegrateInside3(const mesh1D& omeps, const function1D<T0>& f0f1, double x0, double peak0, double peak1,
		    const mesh1D& om2, const function<T2>& f2, double x2, double peak2, const rT&)
{
  // Three separate intervals are defined such that
  // in each interval one of the meshes is most dense and
  // is being used to calculate convolution on that paricular mesh
...
void MakeCombinedFunctions(mesh1D& omeps, function1D<Th>& f0f1,
			   const mesh1D& om0, const function<Tg>& f0, double x0,
			   const mesh1D& om1, const function<Ta>& f1, double x1)
{
...
void MakeCombinedFunctionsInside(mesh1D& omeps, function1D<Th>& f0f1,
				 const mesh1D& om0, const function<T1>& f0, double x0,
				 const mesh1D& om1, const function<T2>& f1, double x1,
