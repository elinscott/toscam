...
!    * DERFCX evaluates the exponentially scaled complementary error function.
...
!    * PSI evaluates the function Psi(X).
...
!    Input, real ( kind = 8 ) X, the argument of the function.
!
!    Output, real ( kind = 8 ) DERFC, the value of the function.
!
  implicit none
...
!    Input, real ( kind = 8 ) X, the argument of the function.
!
!    Output, real ( kind = 8 ) DERFCX, the value of the function.
!
  implicit none
...
!    Input, real ( kind = 8 ) X, the argument of the function.
!
!    Output, real ( kind = 8 ) DERF, the value of the function.
!
  implicit none
...
!
!    Input, integer NMAX, the number of derivatives to compute.
!
...
!
  ...
  if ( irnd == 0 .and. temp - tempa .ne. zero ) then
    irnd = 2
  end if
!
...
!
  ...
  if ( irnd == 0 .and. temp * one - one .ne. zero ) then
    ngrd = 1
  end if
!
...
!
  ...
  if ( a + a == zero .or. abs ( z ) .ge. y ) then
    go to 410
  end if
...
!
  a = y * one
  temp = y * t
  if ( a + a == zero .or. abs ( y ) .ge. xmin ) then
    go to 460
  end if

  k = k + 1
  temp1 = temp * betain
  if ( temp1 * beta .ne. y .or. temp == y ) then
    go to 450
  else
    nxres = 3
    xmin = y
...
!
!      -xmax1 < x < -xmin (x not a negative integer),
!
!    or
!
!      xmin < x.
...
!    Input, real ( kind = 8 ) XX, the argument of the function.
!
!    Output, real ( kind = 8 ) PSI, the value of the function.
!
  implicit none
...
!
  230     continue
      ...
      if ( alpha .ne. zero ) then
        sum = sum * dgamma ( one + alpha ) * ( x * half )**(-alpha)
      end if
...
!
      ...
      if ( alpha .ne. zero ) then
        tempa = halfx**alpha / dgamma ( empal )
      end if
      ...
      if ( x .ne. zero .and. b(1) == zero ) then
        ncalc = 0
      end if
...
!
      ...
      if ( alpha .ne. zero ) then
        tempa = halfx**alpha / ( alpha * dgamma ( alpha ) )
      end if
...
!
  250     continue
      ...
      if ( alpha + one .ne. one ) then
        sum = sum * dgamma ( alpha ) * ( x * half )**( -alpha )
      end if
...
!    0 <= ALPHA < 1.0.
!
!    Input, integer NB, the number of functions to be calculated, NB .GT. 0.
!    The first function calculated is of order ALPHA, and the
!    last is of order (NB - 1 + ALPHA).
!
!    Input, integer IZE, scaling option.
...
!
  ...
  if ( 0 < nb .and. &
    ( enu .ge. zero .and. enu < one ) .and. &
    ( ize .ge. 1 .and. ize <= 2 ) .and. &
    ( ize .ne. 1 .or. ex <= xmax ) .and. &
    zero < ex )  then
...
!
!    Output, integer NCALC, error flag.  Before using the vector BY, the
