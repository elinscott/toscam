...
!    patch is an (X,Y,Z) aimage of the unit rectangle in (U,V) space.
!    Two patches may be said to be "neighbors" if their aimages share
!    a side.
!
...
!
  ...
  call dtable_data_read ( node_file_name, dim_num, node_num, node_xyz )
...
!
  ...
  call dtable_write ( node_file_name, dim_num, node_num, node_xyz, header )
...
!
  ...
  call itable_data_read ( rectangle_file_name, dim_num, rectangle_num, &
    rectangle_node )
...
!
  ...
  call itable_write ( rectangle_file_name, dim_num, rectangle_num, &
...
!
  ...
  write ( output_unit, '(a)'       ) '#  created by TRIANGULATION_ORIENT.F90'
...
subroutine dtable_write ( output_filename, m, n, table, header )
...
!*****************************************************************************80
!
!! I4MAT_PRINT_SOME prints some of an IMAT.
!
!  Discussion:
!
!    An IMAT is a rectangular array of integer values.
...
!*****************************************************************************80
!
!! I4MAT_TRANSPOSE_PRINT_SOME prints some of the transpose of an IMAT.
!
!  Modified:
!
!    09 February 2005
...
subroutine itable_write ( output_filename, m, n, table, header )
...
!
  ...
  if ( ios /= 0 ) then
    ...
    write ( *, '(a)' ) '  Could not open the output file:'
    write ( *, '(a)' ) '  "' // trim ( output_filename ) // '".'
    stop
  end if

  if ( header ) then
    call itable_header_write ( output_filename, output_unit, m, n )
  end if
