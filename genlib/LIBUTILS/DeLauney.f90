...
!             and, for I > 1, LCC(I-1) + 3 .LE. LCC(I)
...
!   distances.
!
  integer, parameter :: n = 1360
  integer, parameter :: ncc_max = 1
  integer, parameter :: nrow = 9
!
  integer, parameter :: lwk = 2 * n
  integer, parameter :: n6 = 6 * n
  integer, parameter :: ntmx = 2 * n
!
  real a
  real areap
  real armax
  ...
  integer k
  integer ksum
  integer, dimension ( ncc_max ) :: lcc = (/ 9 /)
  integer lct(ncc_max)
  integer lend(n)
  ...
  ncc=0
  do i=1,n
   read(12,*,eND=100) x(i),y(i)
  enddo
100 write(*,*) 'please enter a file : fort.12'
...
!
  ...
  wy2 = maxval ( y(1:n) )
!
!  Create an encapsulated PostScript file aimage of the triangulation.
!
...
!
  ...
  call trplot ( lplt, pltsiz, wx1, wx2, wy1, wy2, ncc, lcc, n, x, y, list, lptr, lend, title, numbr, ier )
 
  good=0

  call trprnt ( good, 0, lcc, n, x, y, list, lptr, lend, prntx )
...
!  Parameters:
!
!    Input, integer NCC, the number of constraint curves (constraint 
!    regions).  NCC >= 0.
!
!    Input, integer LCC(NCC) (or dummy array of length 1 if NCC = 0) 
...
!
!    Input, integer N, the number of nodes in the triangulation, including
!    constraint nodes.  N >= 3.
!
!    Input, real X(N), Y(N), the coordinates of the nodes with non-constraint
!    nodes in the first LCC(1)-1 locations, followed by NCC sequences of
!    constraint nodes.  Only one of these sequences may be specified in
!    clockwise order to represent an exterior constraint curve (a 
!    constraint region with nonfinite area).
!
!    Input/output, integer LWK.  On input, the length of IWK.  This must be 
...
!
!    Output, integer IWK(LWK), the endpoint indexes of the new arcs which 
!    were swapped in by the last call to subroutine EDGE.
!
!    Input/output, integerLIST(*), LPTR(*), LEND(N), the data structure 
...
!  Parameters:
!
!    Input, real X(*), Y(*), the Cartesian coordinates of a set of points.
!
!    Input, integer NB, the number of points in the curve.
!
!    Input, integer NODES(NB), the indices of the points that
!    make up the closed curve.
!
!    Output, real AREAP, the signed area bounded by the curve.
!
  implicit none
!
  integer nb
...
!
!    Input, integer IO1, IO2, the indexes (in the range 1 to N) of a pair of
!    adjacent boundary nodes defining the arc to be removed.
!
!    Input/output, integer LIST(*), LPTR(*), LEND(N), LNEW, the
!    triangulation data structure created by TRMESH or TRMSHR.
!    On output, updated with the removal of arc IO1-IO2 unless IER > 0.
!
!    Output, integer IER = Error indicator:
!    0, if no errors were encountered.
...
!
!    Output, integer LPH, list pointer to the hole (NB as a neighbor of
...
!  Parameters:
!
!    Input, integer K, the index (for X and Y) of the node to be deleted.
...
!
!    Input/output, real X(N), Y(N), the coordinates of the nodes with
...
!    that the data structure may have been altered if IER >= 3.
!
...
!  Parameters:
!
!    Input, integer IN1, IN2, indexes (of X and Y) in the range 1 to N
...
!    (or LWK=0) on input.
!
!    Output, integer IWK(2*LWK), the indexes of the endpoints of the new 
...
3   continue
    ...
    if ( left(x2,y2,x1,y1,x(nl),y(nl)) ) go to 4
    lp = lptr(lp)
    nl = list(lp)
...
!
!    Input, integer LCC(*), a list of constraint curve starting indexes (or
!    dummy array of length 1 if NCC = 0).  Refer to subroutine ADDCST.
!
!    Input, integer N, the number of nodes in the triangulation.  N >= 3.
!
!    Input, real X(N), Y(N), the coordinates of the nodes with 
!    non-constraint nodes in the first LCC(1)-1 locations if NCC > 0.
!
!    Input, integer LIST(*), LPTR(*), LEND(N), the triangulation data 
!    structure.  Refer to subroutine TRMESH.
!
!    Input, integer L, the number of nodes in the sequence on output.  
!    2 <= L <= N.
!
!    Input/output, integer NPTS(L), on input, the indexes of the L-1 closest
...
!    IER /= 0.
!
!    Input/output, real DS(L), the distance (defined above) between NPTS(1) 
!    and NPTS(I) in the I-th position for I = 1,...,L-1.  Thus, DS(1) = 0.
!    On output, updated with the distance between NPTS(1) and NPTS(L) in
!    position L unless IER /= 0.
!
!    Output, integer IER = Error indicator:
!     0 if no errors were encountered.
...
12      continue
        ...
        if (j == 1) dc = ds(k) + sqrt((xc-xk)*(xc-xk) &
                           + (yc-yk)*(yc-yk))
13      continue
...
!  Parameters:
!
!    Input, real X1, Y1 = Coordinates of P1.
!
!    Input, real X2, Y2 = Coordinates of P2.
!
!    Input, real X3, Y3 = Coordinates of P3.
!
!    Input, real X4, Y4 = Coordinates of P4.
!
...
!
  intsec = a/d >= 0.0  .and.  a/d <= 1.0  .and. &
           b/d >= 0.0  .and.  b/d <= 1.0
  return
!
...
!
!    Input/output, integer LIST(*), LPTR(*), LEND(N), data structure 
...
!    to be performed.  A reasonable value is 3*NA.  NIT >= 1.  On output, 
!    the number of iterations performed.
!
!    Input/output, integer IWK(2,NA), containing the nodal indexes of the 
...
!  Parameters:
!
!    Input, integer NCC, the number of constraints.
!
!    Input, integer LCT(NCC), the list of constraint triangle starting 
!    indexes (or dummy array of length 1 if NCC = 0).
!
!    Input, integer N, the number of nodes in the triangulation.
!    3 <= N <= 9999.
!
!    Input, real X(N), Y(N), the coordinates of the nodes in the 
...
!    1 <= NT <= 9999.
!
!    Input, integer LTRI(NROW,NT), array whose J-th column contains
...
!  Parameters:
!
!    Input, integer N, the number of nodes in the grid.  N = NX*NY for some
!    NY >= 2.
!
!    Input, integer NX, the number of grid points in the x-direction.  
!    NX >= 2.
!
!    Input, real X(N), Y(N), the coordinates of the nodes with the ordering 
!    and distribution defined in the header comments above.
...
!
!    Output, integer LIST(6*N-12), LPTR(6*N-12), LEND(N), LNEW, data structure
!    defining the triangulation.  Refer to subroutine TRMESH.
!
!    Output, integer IER = Error indicator:
!     0 if no errors were encountered.
...
!  Parameters:
!
!    Input, integer N, the number of nodes.  N .GE. 3.
!
!    Input, real X(N), Y(N), the nodal coordinates.
!
!    Input, integer LIST(*), LPTR(*), LEND(N), the data structure 
!    containing the triangulation.  Refer to subroutine TRMESH.
!
!    Input, real TOL, nonnegative tolerance to allow for floating-point 
!    errors in the circumcircle test.  An error situation is defined as 
!      (R**2 - D**2) / R**2 > TOL, 
...
!    to be 0.
!
!    Output, real ARMAX, maximum aspect ratio (radius of inscribed
...
!
!    Input, real PLTSIZ, the plot size in inches.  The window is mapped,
!    with aspect ratio preserved, to a rectangular viewport with maximum
...
!
!    Input, real WX1, WX2, WY1, WY2, parameters defining a rectangular
!    window against which the triangulation is clipped.  (Only the
...
!
!    Input, integer NCC, the number of constraint curves.  Refer to 
!    subroutine ADDCST.  NCC >= 0.
!
!    Input, integer LCC(NCC) (or dummy parameter if NCC = 0) containing the
...
!
!    Input, real X(N), Y(N), the coordinates of the nodes with non-constraint
!    nodes in the first LCC(1)-1 locations.
!
!    Input, integer LIST(*), LPTR(*), LEND(N), data structure defining the
!    triangulation.  Refer to subroutine TRMESH.
!
...
subroutine trprnt ( good,ncc, lcc, n, x, y, list, lptr, lend, prntx )
!
!*******************************************************************************
!
...
!    triangulation; not used unless PRNTX = TRUE.
!
!    Input, integer LIST(*), LPTR(*), LEND(N), data structure defining 
!    the triangulation.  Refer to subroutine TRMESH.
!
...
!
      ...
      if(k.eq.6.and.nabor(k).ne.0) good(node)=1
      if(k.eq.7.and.nabor(k).eq.0) good(node)=1

      if(good(node).eq.1) write (13,103) node, nabor(1:6)
      if(good(node).eq.1) then
       write (14,*) x(node),y(node)
       do i=1,6
       write(14,*) x(nabor(i)),y(nabor(i))
       enddo
       write(14,*)
      endif
...
!  Increment NL and print X, Y, and NABOR.
      ...
      write (13,*) node,x(node),y(node),(nabor(i), i = 1,k)
    end do
  end if
!  Print NB, NA, and NT (boundary nodes, arcs, and triangles).
  nt = 2*nn - nb - 2
...
!  Print formats:
  ...
  105 format (1x)
  106 format (///)
  107 format (/1x,'nb = ',i4,' boundary nodes',5x, 'na = ',i5,' arcs',5x,'nt = ',i5, ' triangles')
