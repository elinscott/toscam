...
inline void Setv(Amp& A, const function<Amp>& f, int i)
{
  A.m = f[i].m;
  A.p = f[i].p;
}
inline void Seti(Amp& A, const function<Amp>& f, const intpar& ip)
{
...
inline void Setv(Gmpr& G, const function<Gmpr>& f, int i)
{
  G.m = f[i].m;
  G.p = f[i].p;
  G.r = f[i].r;
}
inline void Seti(Gmpr& G, const function<Gmpr>& f, const intpar& ip)
{
...
inline void Setv(Gmpr1& G, const function<Gmpr1>& f, int i)
{
  G.m = f[i].m;
  G.p = f[i].p;
  G.r = f[i].r;
}
inline void Seti(Gmpr1& G, const function<Gmpr1>& f, const intpar& ip)
{
  if (ip.p<0 || ip.p>1) { G.m = G.p = G.r = 0; return;}
  G.m = f[ip.i].m + ip.p*(f[ip.i+1].m - f[ip.i].m);
  G.p = f[ip.i].p + ip.p*(f[ip.i+1].p - f[ip.i].p);
...
inline void Setv(Gmpr2& G, const function<Gmpr2>& f, int i)
{
  G.m = f[i].m;
  G.p = f[i].p;
  G.r = f[i].r;
}
inline void Seti(Gmpr2& G, const function<Gmpr2>& f, const intpar& ip)
{
  if (ip.p<0 || ip.p>1) { G.m = G.p = G.r = 0; return;}
  G.m = f[ip.i].m + ip.p*(f[ip.i+1].m - f[ip.i].m);
  G.p = f[ip.i].p + ip.p*(f[ip.i+1].p - f[ip.i].p);
...
inline void Setv(Hxy& H, const function<Hxy>& f, int i)
{
  H.bh = f[i].bh;
  H.pr = f[i].pr;
  H.pm = f[i].pm;
...
inline void Seti(Hxy& H, const function<Hxy>& f, const intpar& ip)
{
...
inline void Setv(Lxy& L, const function<Lxy>& f, int i)
{
  L.bh = f[i].bh;
  L.mr = f[i].mr;
  L.pr = f[i].pr;
...
inline void Seti(Lxy& L, const function<Lxy>& f, const intpar& ip)
{
...
/******************* Just for debugging *********************/
inline void PrintProduct(ostream& stream, const Hxy& H, const Gmpr& G)
{
  stream <<(H.pr * G.r - H.pm * G.m)<<" "<<(H.bh * G.r + H.pr * G.m)<<" "<<H.mr * G.p<<endl;
}
inline void PrintProduct(ostream& stream, const Gmpr& G, const Hxy& H)
{
  stream <<(H.pr * G.r - H.pm * G.m)<<" "<<(H.bh * G.r + H.pr * G.m)<<" "<<H.mr * G.p<<endl;
}
