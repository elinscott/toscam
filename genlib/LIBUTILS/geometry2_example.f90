...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  Test the routines in the GEOMETRY library.'
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST0045'
  write ( *, '(a)' ) '  ARC_SINE computes an angle with a given sine;'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  X  ARC_SINE(X)  (Degrees)'
  write ( *, '(a)' ) ' '
...
!
  ...
  call r8mat_print ( i4_3, i4_3, u, '  The matrix U' )

  call r8mat_print ( i4_3, i4_3, v, '  The matrix V' )
  ...
  call r8mat_print ( i4_3, i4_3, a, '  The transformation matrix' )

  c(1:3,1:3) = matmul ( a(1:3,1:3), u(1:3,1:3) )

  call r8mat_print ( i4_3, i4_3, c, &
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3,test_num) :: test_t = reshape ( (/ &
   ...
   -2.0D+00, 2.0D+00, &
    ...
    4.0D+00, 2.0D+00, &
    1.0D+00, 5.0D+00, &
    4.0D+00, 2.0D+00 /), (/ dim_num, i4_3, test_num /) )
...
!
  ...
  do test = 1, test_num
    ...
    call r8vec_uniform ( dim_num, p_lo, p_hi, seed, p1 )
    call r8vec_uniform ( dim_num, p_lo, p_hi, seed, p2 )
    call r8vec_uniform ( dim_num, p_lo, p_hi, seed, q1 )
    call r8vec_uniform ( dim_num, p_lo, p_hi, seed, q2 )
...
!
  ...
  do test = 1, test_num
    ...
    call r8vec_uniform ( dim_num, p_lo, p_hi, seed, p1 )
    call r8vec_uniform ( dim_num, p_lo, p_hi, seed, p2 )
    call r8vec_uniform ( dim_num, p_lo, p_hi, seed, p3 )
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST022'
  write ( *, '(a)' ) '  DIRECTION_UNIFORM_3D picks a random direction vector.'
  write ( *, '(a)' ) ' '

  do test = 1, test_num
    call direction_uniform_3d ( seed, vran )
    write ( *, '(2x,3f8.4)' ) vran(1:dim_num)
  end do
...
!*****************************************************************************80
!
!! TEST023 tests DIRECTION_UNIFORM_ND;
!
!  Modified:
!
!    28 July 2006
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST023'
  write ( *, '(a)' ) '  DIRECTION_UNIFORM_ND picks a random direction vector.'
  write ( *, '(a)' ) ' '

  do test = 1, test_num
    call direction_uniform_nd ( dim_num, seed, vran )
...
!
  call icos_size_3d ( point_num1, edge_num1, face_num1, face_order_max1 )
  ...
  call icos_shape_3d ( point_num1, edge_num1, face_num1, face_order_max1, &
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3,test_num) :: t_test = reshape ( (/ &
       ...
       -8.0D+00,  0.0D+00,  0.0D+00, &
        ...
        0.0D+00,  0.0D+00,  0.0D+00, &
        0.0D+00,  4.0D+00,  0.0D+00, &
        0.0D+00,  0.0D+00,  4.0D+00 /), (/ dim_num, i4_3, test_num /) )
  integer ( kind = 4 ) test
  ...
  do test = 1, test_num
    ...
    call r8mat_transpose_print ( dim_num, i4_3, t, '  Triangle vertices' )
...
!
  ...
  call r8mat_transpose_print ( dim_num, i4_6, p, '  Vertices:' )
...
!*****************************************************************************80
!
!! TEST0325 tests ICOS_SIZE_3D and ICOS_SHAPE_3D.
!
!  Modified:
!
!    19 July 2007
...
!
  ...
  write ( *, '(a)' ) '  ICOS_SIZE_3D returns dimension information;'
  write ( *, '(a)' ) '  ICOS_SHAPE_3D returns face and order information.'
  write ( *, '(a)' ) '  SHAPE_PRINT_3D prints this information.'

  call icos_size_3d ( point_num, edge_num, face_num, face_order_max )
...
!
  ...
  do test = 1, test_num
    ...
    call line_exp_perp_2d ( p1, p2, p3, p4 )
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3,test_num) :: t_test = reshape ( (/ &
    ...
    13.0D+00, -4.0D+00, -1.0D+00, &
     ...
     3.0D+00,  0.0D+00, -7.0D+00, &
    13.0D+00, -4.0D+00, -1.0D+00, &
     9.0D+00,  3.0D+00,  8.0D+00 /), (/ dim_num, i4_3, test_num /) )
  integer ( kind = 4 ) test
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3,test_num) :: t_test = reshape ( (/ &
        ...
        -4.0D+00,  1.0D+00,  0.0D+00, &
         ...
         0.0D+00,  0.0D+00,  1.0D+00 /), (/ dim_num, i4_3, test_num /) )
  integer ( kind = 4 ) test
...
!*****************************************************************************80
!
!! TEST067 tests POINTS_CONVEX_HULL_NLOGH_2D.
!
!  Modified:
!
!    12 June 2006
...
!
  ...
  node_xy(1:2,1:node_num) = reshape ( (/ &
    ...
    0.0D+00, 2.0D+00, &
    1.0D+00, 3.0D+00, &
    2.0D+00, 2.0D+00 /), (/ i4_2, node_num /) )

  call r8mat_transpose_print ( i4_2, node_num, node_xy, &
  ...
  call r8mat_transpose_print ( i4_2, hull_num, hull_xy, &
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST068'
  write ( *, '(a)' ) '  POINTS_DIST_SPHERE_3D measures the distance between'
  write ( *, '(a)' ) '  two points on a sphere.'
  write ( *, '(a)' ) ' '
  ...
  do test1 = 1, test_num-1
    ...
    do test2 = test1+1, test_num
      ...
      call points_dist_sphere_3d ( lat1, long1, lat2, long2, radius, dist )

      write ( *, '(a,a,g14.6)' ) '             to ', name(test2), dist
...
!*****************************************************************************80
!
!! TEST0755 tests POLYGON_1_2D, POLYGON_X_2D, POLYGON_Y_2D, POLYGON_XX_2D, POLYGON_XY_2D, POLYGON_YY_2D.
!
!  Modified:
!
!    28 July 2006
...
!
  ...
  real    ( kind = 8 ) area
  real    ( kind = 8 ), dimension(dim_num,4) :: quad = reshape ( (/ &
    0.0D+00, 0.0D+00, &
    1.0D+00, 0.0D+00, &
    1.0D+00, 1.0D+00, &
    0.0D+00, 1.0D+00 /), (/ dim_num, i4_4 /) )
  ...
  call r8mat_transpose_print ( dim_num, i4_4, quad, '  The vertices:' )

  call quad_area_2d ( quad, area )
...
!
  ...
  call r8mat_transpose_print ( dim_num, i4_4, q, '  The vertices:' )
...
!
  ...
  call r8vec_print ( i4_4, q, '  The rotation quaternion:' )
...
!*****************************************************************************80
!
!! TEST1804 tests SIMPLEX_LATTICE_VOLUME_ND.
!
!  Modified:
!
!    09 January 2007
...
!
  ...
  write ( *, '(a)' ) '  SIMPLEX_LATTICE_VOLUME_ND computes the lattice volume,'
  ...
  do test = 1, test_num
    ...
    t = t_test(test)
    volume = simplex_lattice_volume_nd ( dim_num, t )
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,i4_4) :: tetra = reshape ( (/&
     0.000000D+00,  0.942809D+00, -0.333333D+00, &
    -0.816496D+00, -0.816496D+00, -0.333333D+00, &
     0.816496D+00, -0.816496D+00, -0.333333D+00, &
     0.000000D+00,  0.000000D+00,  1.000000D+00 /), (/ dim_num, i4_4 /) )
  real    ( kind = 8 ) volume
  ...
  call r8mat_transpose_print ( dim_num, i4_4, tetra, '  Simplex vertices:' )
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST1837'
  write ( *, '(a)' ) '  SPHERE_IMP_GRID_ICOS_SIZE determines the size'
  write ( *, '(a)' ) '  (number of nodes, number of triangles) in a grid'
  write ( *, '(a)' ) '  on a sphere, made by subdividing an initial'
  write ( *, '(a)' ) '  projected icosahedron.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '  FACTOR determines the number of subdivisions of each'
  write ( *, '(a)' ) '  edge of the icosahedral faces.'
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '    FACTOR     Nodes     Edges Triangles'
  write ( *, '(a)' ) '  --------  --------  --------  --------'
  write ( *, '(a)' ) ' '

  do factor = 1, 10
    call sphere_imp_grid_icos_size ( factor, node_num, edge_num, triangle_num )
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST1838'
  write ( *, '(a)' ) '  SPHERE_IMP_GRID_ICOS_SIZE "sizes" a grid generated'
  write ( *, '(a)' ) '  on an icosahedron and projected to a sphere.'
  write ( *, '(a)' ) '  SPHERE_IMP_GRIDPOINTS_ICOS1 creates the grid points.'
  ...
  call sphere_imp_gridpoints_icos1 ( factor, node_num, node_xyz )
...
!
  if ( .false. ) then
    ...
    write ( file_name, '(a,i1,a)' ) 'sphere_grid_icos1_f', factor, '_xyz.txt'
    ...
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) '  Wrote grid nodes to "' // trim ( file_name ) // '".'
...
!
  ...
  write ( *, '(a)' ) '  SPHERE_IMP_GRID_ICOS_SIZE "sizes" a grid generated'
  write ( *, '(a)' ) '  on an icosahedron and projected to a sphere.'
  write ( *, '(a)' ) '  SPHERE_IMP_GRIDPOINTS_ICOS2 creates the grid.'
  ...
  call sphere_imp_gridpoints_icos2 ( factor, node_num, node_xyz )
...
!
  if ( .false. ) then
    ...
    write ( file_name, '(a,i1,a)' ) 'sphere_grid_icos2_f', factor, '_xyz.txt'
...
!
  ...
  call r8vec_print ( dim_num, pc, '  Center:' )

  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  The number of latitudes =  ', lat_num
  write ( *, '(a,i8)' ) '  The number of longitudes = ', long_num

  call sphere_imp_gridpoints_3d ( r, pc, lat_num, long_num, p )

  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  The number of grid points is ', &
    2 + lat_num * long_num
  write ( *, '(a)' ) ' '

  k = 1
  write ( *, '(2x,i8,3g14.6)' ) k, p(1:dim_num,k)
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  The number of spiral points is ', n

  call sphere_imp_spiralpoints_3d ( r, pc, n, p )

  call r8mat_transpose_print ( dim_num, n, p, '  The spiral points:' )
...
!
  ...
  integer ( kind = 4 ) line(i4_2,line_max)
  ...
  call sphere_imp_gridlines_3d ( line_max, lat_num, long_num, line_num, line )

  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Number of line segments is ', line_num

  call i4mat_transpose_print ( i4_2, line_num, line, '  Grid line vertices:' )
...
!*****************************************************************************80
!
!! TEST1865 tests SPHERE_IMP_GRID_Q4_3D.
!
!  Modified:
!
!    19 July 2006
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST1865'
  write ( *, '(a)' ) '  SPHERE_IMP_GRID_Q4_3D computes a grid'
  ...
  call sphere_imp_grid_q4_3d ( lat_num, long_num, rectangle_node )

  call i4mat_transpose_print ( i4_4, rectangle_num, rectangle_node, &
    '  Rectangle vertices:' )
...
!*****************************************************************************80
!
!! TEST187 tests SPHERE_IMP_GRID_T3_3D.
!
!  Modified:
!
!    11 June 2006
...
!
  ...
  integer ( kind = 4 ), parameter :: i4_3 = 3
  integer ( kind = 4 ), parameter :: triangle_max = 1000
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST187'
  write ( *, '(a)' ) '  SPHERE_IMP_GRID_T3_3D computes a grid'
  write ( *, '(a)' ) '  of T3 triangular elements on a sphere in 3D.'
  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  Number of latitudes is  ', lat_num
  write ( *, '(a,i8)' ) '  Number of longitudes is ', long_num

  call sphere_imp_grid_t3_3d ( triangle_max, lat_num, long_num, triangle_num, &
    triangle_node )

  write ( *, '(a)' ) ' '
  write ( *, '(a,i8)' ) '  The number of triangles is ', triangle_num

  call i4mat_transpose_print ( i4_3, triangle_num, triangle_node, &
    '  Triangle vertices:' )
...
!
  ...
  do test = 1, test_num
    ...
    call r8vec_uniform ( 3, a, b, seed, xyz1 )
...
!
  ...
  call r8mat_transpose_print ( dim_num, i4_4, tetra, '  Tetrahedron vertices:' )
...
!
  ...
  real    ( kind = 8 ), dimension(4,test_num) :: c_test = reshape ( (/ &
    ...
    -1.3D+00, 2.0D+00,  0.2D+00, 0.1D+00, &
     0.8D+00, 0.6D+00, -0.5D+00, 0.1D+00 /), (/ i4_4, test_num /) )
  logical inside
  real    ( kind = 8 ) p(dim_num)
  integer ( kind = 4 ) test
  real    ( kind = 8 ), dimension(dim_num,4) :: tetra = reshape ( (/&
...
!
  ...
  call r8vec_print ( i4_6, edge_length, '  Edge lengths:' )
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,4) :: t = reshape ( (/ &
     ...
     1.0D+00, 4.0D+00, 4.0D+00 /), (/ dim_num, i4_4 /) )
  integer ( kind = 4 ) test
  ...
  call r8mat_transpose_print ( dim_num, i4_4, t, '  Tetrahedron vertices' )
...
!
  ...
  integer ( kind = 4 ) test
  integer ( kind = 4 ), parameter :: test_num = 10
  real    ( kind = 8 ) theta_range
  real    ( kind = 8 ) theta1
  real    ( kind = 8 ) theta2

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST2035'
  write ( *, '(a)' ) '  THETA2_ADJUST tries to "adjust" the THETA coordinates'
  write ( *, '(a)' ) '  of two points by adding 2 PI where necessary, to '
  write ( *, '(a)' ) '  reduce the range, that is, MAX(T1,T2) - MIN(T1,T2).'

  r8_lo = 0.0D+00
  r8_hi = 2.0D+00 * pi

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '     Index              Theta1      Theta2      Range'
  write ( *, '(a)' ) ' '
  ...
  do test = 1, test_num
    ...
    theta_range = max ( theta1, theta2 ) &
                - min ( theta1, theta2 )

    write ( *, '(a)' ) ' '

    write ( *, '(2x,i8,2x,a,2x,f10.4,2x,f10.4,2x,f10.4)' ) &
      test, 'Before', theta1, theta2, theta_range

    call theta2_adjust ( theta1, theta2 )

    theta_range = max ( theta1, theta2 ) &
                - min ( theta1, theta2 )

    write ( *, '(2x,i8,2x,a,2x,f10.4,2x,f10.4,2x,f10.4)' ) &
      test, 'After ', theta1, theta2, theta_range
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) 'TEST2036'
  write ( *, '(a)' ) '  THETA3_ADJUST tries to "adjust" the THETA coordinates'
  write ( *, '(a)' ) '  of three points by adding 2 PI where necessary, to '
  write ( *, '(a)' ) '  reduce the range, that is, MAX(T1,T2,T3) - MIN(T1,T2,T3).'

  r8_lo = 0.0D+00
  r8_hi = 2.0D+00 * pi

  write ( *, '(a)' ) ' '
  write ( *, '(a)' ) '     Index              Theta1      Theta2      Theta3       Range'
  write ( *, '(a)' ) ' '
  ...
  do test = 1, test_num
    ...
    theta_range = max ( theta1, theta2, theta3 ) &
                - min ( theta1, theta2, theta3 )

    write ( *, '(a)' ) ' '

    write ( *, '(2x,i8,2x,a,2x,f10.4,2x,f10.4,2x,f10.4,2x,f10.4)' ) &
      test, 'Before', theta1, theta2, theta3, theta_range

    call theta3_adjust ( theta1, theta2, theta3 )

    theta_range = max ( theta1, theta2, theta3 ) &
                - min ( theta1, theta2, theta3 )

    write ( *, '(2x,i8,2x,a,2x,f10.4,2x,f10.4,2x,f10.4,2x,f10.4)' ) &
      test, 'After ', theta1, theta2, theta3, theta_range
...
!
  ...
  call r8mat_print ( i4_4, i4_4, a, '  Initial transformation matrix:' )
!
...
!
  ...
  call r8mat_transpose_print ( i4_3, n, point, '  Points:' )
!
...
!
  ...
  call r8mat_transpose_print ( i4_3, n, point2, ' ' )
!
!  Rotation about a vector.
!
  angle = 30.0D+00
...
!
  ...
  real    ( kind = 8 ), dimension ( dim_num, 3 ) :: t = reshape ( (/ &
    ...
    1.7071068D+00, 2.7071068D+00, 4.0D+00 /), (/ dim_num, i4_3 /) )
...
!
  ...
  real    ( kind = 8 ), dimension ( dim_num, 3 ) :: t = reshape ( (/ &
    1.0D+00, 0.0D+00, 0.0D+00, &
    0.0D+00, 1.0D+00, 0.0D+00, &
    0.0D+00, 0.0D+00, 1.0D+00 /), (/ dim_num, i4_3 /) )
  ...
  call r8vec_print ( i4_3, s, '  Side lengths:' )
...
!
  ...
  call r8mat_print ( dim_num, i4_3, t, '  Triangle (vertices are columns)' )
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3,test_num) :: t_test = reshape ( (/ &
         ...
         0.0D+00,  0.0D+00, &
         1.0D+00,  0.0D+00, &
        10.0D+00,  2.0D+00 /), (/ dim_num, i4_3, test_num /) )
  integer ( kind = 4 ) test
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3,test_num) :: t_test = reshape ( (/ &
         ...
         10.0D+00,  5.0D+00, &
         11.0D+00,  5.0D+00, &
        20.0D+00,   7.0D+00 /), (/ dim_num, i4_3, test_num /) )
  integer ( kind = 4 ) test
  ...
  write ( *, '(a)' ) '  TRIANGLE_CIRCUMCENTER_2D_2 computes the circumcenter.'
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3) :: t = reshape ( (/  &
    ...
    2.0D+00, 1.0D+00, 2.0D+00 /), (/ dim_num, i4_3 /) )
...
!
  ...
  call r8mat_transpose_print ( dim_num, i4_3, t2, &
...
!
  ...
  real    ( kind = 8 ), dimension(dim_num,3,test_num) :: t_test = reshape ( (/ &
    ...
    -2.0D+00,  2.0D+00, &
     ...
     1.0D+00,  5.0D+00, &
     4.0D+00,  2.0D+00, &
     1.0D+00,  5.0D+00 /), (/ dim_num, i4_3, test_num /) )
  integer ( kind = 4 ) test
...
!
  ...
  do test = 1, test_num
    ...
    call triangle_orthocenter_2d ( t, pc )
...
!
  ...
  integer ( kind = 4 ), parameter :: dim_num = 2
  integer ( kind = 4 ), parameter :: i4_19 = 19
  integer ( kind = 4 ), parameter :: test_num = 4
...
!
  ...
  call i4mat_transpose_print ( i4_3, n, v, '  The voxels:' )
