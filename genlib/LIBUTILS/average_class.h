...
class AvFun{
  ...
  T simplev;
public:
  AvFun(const function<T>& f0, const mesh& om);
  AvFun();
  void SetUp(const function<T>& f0, const mesh& om);
...
template <class T>
inline void AvFun<T>::SetUp(const function<T>& f0, const mesh& om)
{
  fi.resize(om.size());
  if (om.size()!=f0.size()) std::cerr<<"Function and Mesh not compatible in AvFun"<<endl;
  fi[0][0] = f0[0];
  fi[0][1] = fi[0][2]=0;
...
template <class T>
inline AvFun<T>::AvFun(const function<T>& f0, const mesh& om) : fi(om.size())
{ SetUp(f0,om);}

template <class T>
inline AvFun<T>::AvFun(){};
