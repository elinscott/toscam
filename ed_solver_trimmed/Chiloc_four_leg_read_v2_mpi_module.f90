module mpirout
 ...
 !--------------!
   ...
   logical :: MPIseparate,enable_mpi_dot,nompi=.false.
 !--------------!
...
!--------------------------------------------------------------------------!
INTERFACE mpibcast
    MODULE PROCEDURE mpibcast_,mpibcast__,mpibcast___,mpibcast____,mpibcastb_,mpibcastb__,mpibcastb___,mpibcastb____, &
                   & mpibcast_2arrays_r,mpibcast_2arrays_c,mpibcast_no_disp_r,mpibcast_no_disp_c,mpibcastbcomp_,mpibcastbcomp_2d,mpibcastb_2d
END INTERFACE
!--------------------------------------------------------------------------!
INTERFACE mpierror
...
!--------------------------------------------------------------------------!
INTERFACE mpisum
    MODULE PROCEDURE adim5s,adim1s,adim2s,adim3s,ardim1s,ardim2s,ardim3s,adim4s,ardim4s,adim0,adim0r,adim0_,adim0r_,adim0i_
END INTERFACE
!--------------------------------------------------------------------------!
...
!*********************************************
 ...
 if(.not.MPIseparate.and.enable_mpi_dot)then
if(size2==1)then
...
real(8),dimension(:),pointer           :: c => null()
 ...
 call MPI_AllGatherv(c,size(c),MPI_DOUBLE_PRECISION,a,v,w,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr)
 c => null()
...
complex(8),dimension(:)  :: a,source
 ...
 call MPI_Gather(source,size(source),MPI_DOUBLE_COMPLEX,a, &
               & MPI_DOUBLE_COMPLEX,0,MPI_COMM_WORLD,ierr)
end subroutine
...
complex(8),dimension(:),pointer        :: c => null()
 if(size2==1)return
 c => a(1+w(rank+1):w(rank+1)+v(rank+1))
 call MPI_AllGatherv(c,size(c),MPI_DOUBLE_COMPLEX,a,v,w,MPI_DOUBLE_COMPLEX,MPI_COMM_WORLD,ierr)
 c => null()
end subroutine
...
integer :: unit,ier
real(8)  :: number
 if(rank==0) read(unit,*,end=66) number
 66 continue
...
real(8)  :: number,number2
 if(rank==0) read(unit,*,end=66) number,number2
 66 continue
 if(size2>1) call MPI_BCAST(number,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
 if(size2>1) call MPI_BCAST(number2,1,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ierr)
end subroutine
...
integer  :: number
 if(rank==0) read(unit,*,end=66) number
 66 continue
 if(size2>1) call MPI_BCAST(number,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
end subroutine
...
integer  :: number,number2
 ...
 66 continue
 if(size2>1) call MPI_BCAST(number,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
 if(size2>1) call MPI_BCAST(number2,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
end subroutine
...
logical  :: number
 if(rank==0) read(unit,*,end=66) number
 66 continue
 if(size2>1) call MPI_BCAST(number,1,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
end subroutine
...
complex(8)  :: number
 if(rank==0) read(unit,*,end=66) number
 66 continue
 if(size2>1) call MPI_BCAST(number,1,MPI_DOUBLE_COMPLEX,0,MPI_COMM_WORLD,ierr)
end subroutine
...
implicit none
complex(8) :: mat(:,:,:,:,:),matm(size(mat(:,1,1,1,1)),size(mat(1,:,1,1,1)),size(mat(1,1,:,1,1)),size(mat(1,1,1,:,1)),size(mat(1,1,1,1,:)))
integer    :: i,j,k,s(5),p
if(size2==1)then
 return
endif
...
implicit none
 ...
 if(maxval(abs(shape(mat)-shape(matm2)))>1.d-4) stop 'error ardim3_mask, bad shapes'
 s=shape(mat)
 p=s(1)*s(2)*s(3)
