...
!**************************************************************************
   ...
   SUBROUTINE DUMP_OUTPUT_TEST(greenAB,greenBA,BA)
    ...
    integer                            :: iorb,jorb,ij,ji,ipm,si,ssi,sssi,i,j,k,ssi_,si_,sssi_
...
if(.false.)then
      ...
      do k=1,8
      if(i/=j.and.j/=k)then
      if(    abs(abs(xxx(i))-abs(xxx(j)))>1.d-3 .and. abs(abs(xxx(j))-abs(xxx(k)))>1.d-3 .and. &
       ...
       do ssi_ = -2,2
       do sssi_= -2,2
         aaa=abs(  cmplx(dble(si  ),dble(si_  ))*xxx(i) +  &
                 & cmplx(dble(ssi ),dble(ssi_ ))*xxx(j) -  &
                 & cmplx(dble(sssi),dble(sssi_))*xxx(k)   )
         if( abs(aaa) < 1.d-5 .and. abs(si) + abs(ssi) + abs(sssi)>0 ) then
          write(96,*) '-------------------------------------'
          write(96,*) 'elements are equal : ', i,j,k 
          write(96,*)
          write(96,*) 'x1                 : ', xxx(i),si,si_
          write(96,*) '   +  x2           : ', xxx(j),ssi,ssi_
          write(96,*) '         =   x3    : ', xxx(k),sssi,sssi_
          write(96,*) 'difference         : ', aaa
         endif
       enddo
       enddo
       enddo
      ...
      if(.true.)then
       do ipm=1,2
        if(BA) CALL transform_correl(greenBA%correl(ipm,ipm),greenBA%correl(ipm,ipm))
               CALL transform_correl(greenAB%correl(ipm,ipm),greenAB%correl(ipm,ipm))
       enddo
        write(96,*) 'link  : ', iorb,jorb
        write(96,*) ' ---------------AB----------------'
        write(96,*) 'ij -- : ', greenAB%correl(1,1)%vec(ij,4)
