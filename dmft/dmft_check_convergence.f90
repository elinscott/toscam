...
real(8)                :: beta
 ...
 if(paramagnet==1) mmax=1
   ...
   do kk_=1,mmax
     ...
     if(.not.allocated(temp)) allocate(temp(fac*channels),vvv(channels,2),COEF(channels,2),frequ_(n_frequ),green_lat(n_frequ,channels,2),sigma_lat(n_frequ,channels,2))
     read(10012) vvv(:,kk_),mmu,COEF(:,kk_)
     do i=1,n_frequ
   ...
   write(*,*) 'Chem          : ', mmu
   write(*,*) 'Delta(oo) up  : ', vvv(:,1)
   write(*,*) 'Sig(oo)   up  : ', sigma_lat(n_frequ,:,1)
   write(*,*) 'beta          : ', beta
   ...
   allocate(tt(channels,n_frequ_long),ddens(channels))
 
   do kk_=1,mmax

   do k=1,channels
    aa       = aimag(frequ_(n_frequ-7))**2*real(green_lat(n_frequ-7,k,kk_))
    aa_matsu = aimag(frequ_(n_frequ-7))**2*real(green_matsu(n_frequ-7,k,kk_))
    write(*,*) 'EIMP LAT : ', k,aa
    write(*,*) 'EIMP IMP : ', k,aa_matsu
    ...
    do i=1,n_frequ
        tt(k,i)  = 1.d0 / (   1.d0/green_matsu(i,k,kk_) + (frequ_(i)+mmu)*(COEF(k,kk_)-1.d0)  )
   ...
   if(n_frequ/=n_frequ_long)then
    ...
    do k=1,channels
      ddens(k)=get_dens(real(tt(k,:)),aimag(tt(k,:)),diag=.true.)*COEF(k,kk_)
      write(102,*) ddens(k)
    enddo
