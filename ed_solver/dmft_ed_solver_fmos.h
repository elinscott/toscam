...
!**************************************************************************
 ...
 implicit none
 integer                :: Nc,Nb,s1,s2,iter,i,j,k,l,m,kp,mp,ip,jp,nw_r,nw_m,Ntot,lp,ii,jj
 real(8),allocatable    :: Eb(:,:,:),Ei(:,:,:),Ei_(:,:,:)
 complex(8),allocatable :: Vb(:,:,:),Vbath(:,:,:)
 real(8),allocatable    :: densav(:,:,:),ueff(:,:),Umma(:,:),Ummb(:,:,:),Ummc(:,:,:),UUeff(:,:,:,:)
 complex(8),allocatable :: SIAM_T(:,:,:)
 complex(8),allocatable :: I1a(:,:,:),I1b(:,:,:,:),I2a(:,:,:),I2b(:,:,:,:),I1c(:,:,:,:),I2c(:,:,:,:)
 complex(8),allocatable :: Gdyn_m(:,:,:,:),Gdyn_r(:,:,:,:)
 complex(8),allocatable :: G_ff_m(:,:,:,:),G_ff_r(:,:,:,:),S_ff_m_old(:,:,:,:),S_ff_m(:,:,:,:),S_ff_r(:,:,:,:)
 complex(8),allocatable :: Dmasig(:,:,:),Dmbsig(:,:,:,:),Dmcsig(:,:,:,:),DmasigTild(:,:,:),DmbsigTild(:,:,:,:),DmcsigTild(:,:,:,:)
 ...
 logical                :: fmos_fluc_
   ...
   allocate(Eb(-1:1,Nb,Nb),Vb(-1:1,Nc,Nb),Vbath(-1:1,Nc,Nb/Nc),Ei_(-1:1,Nc,Nc),Ei(-1:1,Nc,Nc),matsubara(nw_m),UU(-1:1,-1:1,Nc,Nc))
   ...
   allocate(G_ff_m(-1:1,Nc,Nc,nw_m),G_ff_r(-1:1,Nc,Nc,nw_r),S_ff_m(-1:1,Nc,Nc,nw_m),S_ff_r(-1:1,Nc,Nc,nw_r))
   if(allocated(densav)) deallocate(densav,ueff,Umma,Ummb,Ummc,UUeff,Gdyn_m,Gdyn_r)
   allocate(densav(-1:1,Nc+Nb,Nc+Nb),ueff(-1:1,Nc),UUeff(-1:1,-1:1,Nc,Nc))
   allocate(S_ff_m_old(-1:1,Nc,Nc,nw_m),Gdyn_m(-1:1,Nc+Nb,Nc+Nb,nw_m),Gdyn_r(-1:1,Nc+Nb,Nc+Nb,nw_r))
   allocate(Umma(-1:1,Nc),Ummb(-1:1,Nc,Nc),Ummc(-1:1,Nc,Nc))
   if(allocated(I1a)) deallocate(I1a,I1b,I2a,I2b,I1c,I2c)
   allocate(I1a(-1:1,Nc,nw_m),I1b(-1:1,Nc,Nc,nw_m),I2a(-1:1,Nc,nw_m),I2b(-1:1,Nc,Nc,nw_m),I1c(-1:1,Nc,Nc,nw_m),I2c(-1:1,Nc,Nc,nw_m)) 
   allocate(Dmasig(-1:1,Nc,nw_m),Dmbsig(-1:1,Nc,Nc,nw_m),Dmcsig(-1:1,Nc,Nc,nw_m),DmasigTild(-1:1,Nc,nw_m),DmbsigTild(-1:1,Nc,Nc,nw_m),DmcsigTild(-1:1,Nc,Nc,nw_m))
   allocate(Dmsig(-1:1,Nc,nw_m))
   allocate(AA(-1:1,Nc,nw_m),BB(-1:1,Nc,Nc,nw_m),CC(-1:1,Nc,Nc,nw_m),tempvec(nw_m))
   ...
   do j=1,Nc
     ...
     if(s1==s2.and.i/=j)  UU(s1,s2,i,j) = (UUmatrix(i,j) - 2.0*JJmatrix(i,j)) 
     if(s2/=s1.and.i/=j)  UU(s1,s2,i,j) = (UUmatrix(i,j) -     JJmatrix(i,j)) 
   enddo
   enddo
   enddo
   enddo
...
S_ff_r=0.
   ...
   do s1=-1,1,2
     call write_array(Eb(s1,:,:),'Epsilon matrix',unit=6,ultrashort=.true.)
     call write_array(real(Vb(s1,:,:)),'V matrix',unit=6,short=.true.)
     call write_array(real(Vbath(s1,:,:)),'V matrix packed',unit=6,short=.true.)
     call write_array(Ei_(s1,:,:),'Eimp matrix',unit=6,ultrashort=.true.)
   enddo
   ...
   !------------------!
    ...
    !END BUG
      ...
      do s1=-1,1,2
        ...
        if(maxval(abs( SIAM_T(s1,:,:) - transpose(conjg( SIAM_T(s1,:,:) )) ))>1.d-4)then
          write(*,*) 'ERROR FMOS SIAM_T not HERMITIC'
          stop
        endif
        do i=1,nw_m
          Gdyn_m(s1,1:Ntot,1:Ntot,i) = (matsubara(i)*imi+mmu)*Id(Ntot)  - SIAM_T(s1,1:Ntot,1:Ntot)
          Gdyn_m(s1,1:Nc,1:Nc,i)=Gdyn_m(s1,1:Nc,1:Nc,i) - S_ff_m(s1,1:Nc,1:Nc,i)
     ...
     do s1=-1,1,2
      do i=1,Nc+Nb
       do j=1,Nc+Nb
       if(.false.)then
         densav(s1,i,j)=1.d0/beta_ED*sum(real(Gdyn_m(s1,i,j,:)))*2.d0
         if(i==j) densav(s1,i,j)=densav(s1,i,j)+0.5d0
       else
         call Fourier(nw_m,Gdyn_m(s1,i,j,1:nw_m),matsubara(1:nw_m),densav(s1,i,j),beta_ED,tailmax=3.d0,green=i==j)
       endif
       enddo
       if(rank==0.and.s1==1) write(*,*) 'FMOS DENSITY :',i,s1,densav(s1,i,i)
       if(rank==0.and.i<=Nc.and.s1==1)  write(*,*) 'MAX HOPPING VAL : ', maxval(abs(densav(s1,i,Nc+1:Ntot)))
       if(rank==0.and.i<=Nc.and.s1==1)  write(*,*) 'MAX HOPPING LOC : ', maxloc(abs(densav(s1,i,Nc+1:Ntot)))

      enddo
     enddo
     write(*,*) 'TOTAL DENSITY : ', sum(diag(densav(-1,1:Nc,1:Nc))+diag(densav(+1,1:Nc,1:Nc)))
    ...
    if(.not.fmos_fluc_)then
      ...
      do s1=-1,1,2
       do m=1,Nc
        ...
        do l=1,Nc
         if(m/=l)then
          temp=temp+UU(s1, s1,l,m)*densav( s1,l,l)*densav(s1,m,m)+ &
                  & UU(s1,-s1,l,m)*densav(-s1,l,l)*densav(s1,m,m)
         endif
        enddo
   ...
   do s1=-1,1,2
    do m=1,Nc
      ...
      do l=1,Nc
       if(l/=m) ueff(s1,m)=ueff(s1,m) + UU(s1,s1,l,m)*densav(s1,l,l) + UU(s1,-s1,l,m)*densav(-s1,l,l)
      enddo
    enddo
   enddo
   ...
   do m=1,Nc
   if(l/=m)then
     UUeff(s1,s1,l,m)=UU(s1,s1,l,m)+UU(1,-1,m,m)*densav(-s1,m,m)+UU(s1,-s1,l,m)*densav(-s1,l,l)
     do lp=1,Nc
      if(lp/=m.and.lp/=l)then
         UUeff(s1,s1,l,m)=UUeff(s1,s1,l,m)+ UU(s1,s1,lp,m)*densav(s1,lp,lp) +UU(s1,-s1,lp,m)*densav(-s1,lp,lp)
      endif
     enddo
     UUeff(s1,-s1,l,m)=UU(s1,-s1,l,m)+UU(1,-1,m,m)*densav(-s1,m,m)+UU(s1,s1,l,m)*densav(s1,l,l)
     do lp=1,Nc
   ...
   write(*,*) 'Diag ueff -1 : ', ueff(-1,:)
   call write_array(UUeff(1, 1,:,:),'Ueff ++',unit=6,short=.true.)
   call write_array(UUeff(1,-1,:,:),'Ueff +-',unit=6,short=.true.)
  ...
  if(.not.fmos_fluc_)then
   Umma=0.
    do s1=-1,1,2
     do m=1,Nc
       ...
       do l=1,Nc
        Umma(s1,m)=Umma(s1,m)+2.d0*(UU(s1,s1,l,m)*densav(s1,l,l)+UU(s1,-s1,l,m)*densav(-s1,l,l) )
       enddo
     enddo
    enddo
  else
    ...
    do s1=-1,1,2
     do m=1,Nc
       ...
       do l=1,Nc
        if(l/=m)then
         Umma(s1,m)=Umma(s1,m)+2.d0* ( &
                  & UU(s1,s1,l,m)*densav(s1,l,l)+UU(s1,-s1,l,m)*densav(-s1,l,l) + &
                  & 2.d0*UU(1,-1,l,l)*densav(s1,l,l)*densav(-s1,l,l) )
        endif
       enddo
     enddo
    enddo
    ...
    do s1=-1,1,2
     ...
     do m=1,Nc
      if(l/=m)then
       ...
       Ummb(s1,l,m)= Ummb(s1,l,m)+2.d0* ( &
               & UU(1,-1,m,m)*densav(-s1,m,m)+UU(s1,-s1,l,m)*densav(-s1,l,l) + &
               & UU(1,-1,m,m)*densav( s1,m,m)*densav(-s1,m,m)+UU(1,-1,l,l)*densav(s1,l,l)*densav(-s1,l,l) ) 
         do lp=1,Nc
           if(lp/=l.and.lp/=m)then
               Ummb(s1,l,m)=Ummb(s1,l,m)+ 2.d0*( &
               &    UU(s1,s1,lp,m)*densav(s1,lp,lp)+ UU(s1,-s1,lp,m)*densav(-s1,lp,lp)+ &
               &             2.d0*UU(1,-1,lp,lp)*densav(s1,lp,lp)*densav(-s1,lp,lp) &                   
               & ) 
           endif
         enddo
       endif
    ...
    enddo
     ...
     !-------------------------------------------------------------!
      ...
      do m=1,Nc
       Dmsig(s1,m,:)=0.
       do k=1,Nb
        Dmsig(s1,m,:) = Dmsig(s1,m,:) + conjg(Vb(s1,m,k)) * Vb(s1,m,k) / &
                   & (  matsubara(:)*imi+mmu-Eb(s1,k,k)  )
       enddo
      enddo
      enddo
      ...
      do m=1,Nc
       Dmasig(s1,m,:)=0.
       do k=1,Nb
        Dmasig(s1,m,:) = Dmasig(s1,m,:) + conjg(Vb(-s1,m,k)) * Vb(-s1,m,k) / &
                   & ( matsubara(:)*imi+mmu+Ei(-s1,m,m)-Ei(s1,m,m)-Eb(-s1,k,k)  )
       enddo
      enddo
      enddo
      ...
      do l=1,Nc
       Dmbsig(s1,l,m,:)=0.
       do k=1,Nb
        Dmbsig(s1,l,m,:) = Dmbsig(s1,l,m,:) + conjg(Vb(s1,l,k)) * Vb(s1,l,k) / &
                   & ( matsubara(:)*imi+mmu+Ei(s1,l,l)-Ei(s1,m,m)-Eb(s1,k,k)  )
       enddo
       enddo
      enddo
      enddo
      ...
      do l=1,Nc
       Dmcsig(s1,l,m,:)=0.
       do k=1,Nb
        Dmcsig(s1,l,m,:) = Dmcsig(s1,l,m,:) + conjg(Vb(-s1,l,k)) * Vb(-s1,l,k) / &
                   & ( matsubara(:)*imi+mmu+Ei(-s1,l,l)-Ei(s1,m,m)-Eb(-s1,k,k)  )
       enddo
       enddo
      enddo
      ...
      do m=1,Nc
       DmasigTild(s1,m,:)=0.
       do k=1,Nb
        DmasigTild(s1,m,:) = DmasigTild(s1,m,:) + conjg(Vb(-s1,m,k)) * Vb(-s1,m,k) / &
                   & ( matsubara(:)*imi+mmu + Eb(-s1,k,k) - Ei(-s1,m,m) - Ei(s1,m,m) - Umma(s1,m)   )
       enddo
      enddo
      enddo
      ...
      do l=1,Nc
       DmbsigTild(s1,l,m,:)=0.
       do k=1,Nb
        DmbsigTild(s1,l,m,:) = DmbsigTild(s1,l,m,:) + conjg(Vb(s1,l,k)) * Vb(s1,l,k) / &
                   & ( matsubara(:)*imi+mmu + Eb(s1,k,k) - Ei(s1,l,l) - Ei(s1,m,m) - Ummb(s1,l,m)   )
       enddo
      enddo
      enddo
      enddo
      ...
      do l=1,Nc
       DmcsigTild(s1,l,m,:)=0.
       do k=1,Nb
        DmcsigTild(s1,l,m,:) = DmcsigTild(s1,l,m,:) + conjg(Vb(-s1,l,k)) * Vb(-s1,l,k) / &
                   & ( matsubara(:)*imi+mmu + Eb(-s1,k,k) - Ei(-s1,l,l) - Ei(s1,m,m) - Ummc(s1,l,m)   )
       enddo
      enddo
      enddo
      enddo
    ...
    do m=1,Nc
     I1a(s1,m,:)=0.
     do k=1,Nb
      I1a(s1,m,:) = I1a(s1,m,:) + conjg(Vb(-s1,m,k)) * densav(-s1,m,Nc+k) / &
                 & ( matsubara(:)*imi+mmu+Ei(-s1,m,m)-Ei(s1,m,m)-Eb(-s1,k,k)  )
      I1a(s1,m,:) = I1a(s1,m,:) -        Vb(-s1,m,k) * densav(-s1,Nc+k,m) / &
                 & ( matsubara(:)*imi+mmu+Eb(-s1,k,k)-Ei(-s1,m,m)-Ei(s1,m,m)-Umma(s1,m)  )
     enddo
    enddo
    enddo 
   ...
   do s1=-1,1,2
    do m=1,Nc
     ...
     do k=1,Nb
     do kp=1,Nb
      I2a(s1,m,:) = I2a(s1,m,:) - conjg(Vb(-s1,m,k)) * Vb(-s1,m,k) * densav(-s1,Nc+kp,Nc+k) / &
                 & ( matsubara(:)*imi+mmu+Ei(-s1,m,m)-Ei(s1,m,m)-Eb(-s1,k,k)  )
      I2a(s1,m,:) = I2a(s1,m,:) - conjg(Vb(-s1,m,k)) * Vb(-s1,m,k) * densav(-s1,Nc+k,Nc+kp) / &
                 & ( matsubara(:)*imi+mmu+Eb(-s1,k,k)-Ei(-s1,m,m)-Ei(s1,m,m)-Umma(s1,m)  )
     enddo
     enddo
    enddo
    ...
    do l=1,Nc
      I1b(s1,m,l,:) = 0.
     do k=1,Nb
      I1b(s1,m,l,:) = I1b(s1,m,l,:) + conjg(Vb(s1,l,k)) * densav(s1,l,Nc+k) / &
                 & ( matsubara(:)*imi + mmu + Ei(s1,l,l) - Ei(s1,m,m) - Eb(s1,k,k)  )
      I1b(s1,m,l,:) = I1b(s1,m,l,:) -       Vb(s1,l,k) * densav( s1,Nc+k,l) / &
                 & ( matsubara(:)*imi + mmu + Eb(s1,k,k) - Ei(s1,l,l) - Ei(s1,m,m) - Ummb(s1,l,m)  )
     enddo
    enddo
    enddo
    ...
    do l=1,Nc
     ...
     do k=1,Nb
     do kp=1,Nb
      I2b(s1,m,l,:) = I2b(s1,m,l,:) - conjg(Vb(s1,l,k)) * Vb(s1,l,k) * densav(s1,Nc+kp,Nc+k) / &
                 & ( matsubara(:)*imi+mmu+Ei(s1,l,l)-Ei(s1,m,m)-Eb(s1,k,k)  )
      I2b(s1,m,l,:) = I2b(s1,m,l,:) - conjg(Vb(s1,l,k)) * Vb(s1,l,k) * densav(s1,Nc+k,Nc+kp) / &
                 & ( matsubara(:)*imi+mmu+Eb(s1,k,k)-Ei(s1,m,m)-Ei(s1,l,l)-Ummb(s1,l,m)  )
     enddo
     enddo
    enddo
    enddo
    ...
    do l=1,Nc
     ...
     do k=1,Nb
      I1c(s1,m,l,:) = I1c(s1,m,l,:) + conjg(Vb(-s1,l,k)) * densav(-s1,l,Nc+k) / &
                 & ( matsubara(:)*imi+mmu+Ei(-s1,l,l)-Ei(s1,m,m)-Eb(-s1,k,k)  )
      if(.not.use_def_feng)then
       I1c(s1,m,l,:) = I1c(s1,m,l,:) -       Vb(-s1,l,k) * densav(-s1,Nc+k,l) / &
                  & ( matsubara(:)*imi+mmu+Eb(-s1,k,k)-Ei(-s1,l,l)-Ei(s1,m,m)-Ummc(s1,l,m)  )
      endif
     enddo
     if(use_def_feng)then
      do k=1,Nb/Nc
        i=(m-1)*mstep 
        j=(l-1)*mstep
        I1c(s1,m,l,:) = I1c(s1,m,l,:) - Vbath(-s1,m,k) * densav(-s1,Nc+k+j,l) / &
                 & ( matsubara(:)*imi+mmu+Eb(-s1,k+j,k+j)-Ei(-s1,l,l)-Ei(s1,m,m)-Ummc(s1,l,m)  )
      enddo
     endif
    ...
    do l=1,Nc
     ...
     do k=1,Nb
     do kp=1,Nb
      I2c(s1,m,l,:) = I2c(s1,m,l,:) - conjg(Vb(-s1,l,k)) * Vb(-s1,l,k) * densav(-s1,Nc+kp,Nc+k) / &
                 & ( matsubara(:)*imi+mmu+Ei(-s1,l,l)-Ei(s1,m,m)-Eb(-s1,k,k)  )
      I2c(s1,m,l,:) = I2c(s1,m,l,:) - conjg(Vb(-s1,l,k)) * Vb(-s1,l,k) * densav(-s1,Nc+k,Nc+kp) / &
                 & ( matsubara(:)*imi+mmu+Eb(-s1,k,k)-Ei(-s1,l,l)-Ei(s1,m,m)-Ummc(s1,l,m)  )
     enddo
     enddo
    enddo
    ...
    if(fmos_fluc_)then
      ...
      do m=1,Nc
       ...
       do l=1,Nc
        if(l/=m)then
         AA(s1,m,:) = AA(s1,m,:) + UU(s1,s1,l,m)*densav(s1,l,l)+UU(s1,-s1,l,m)*densav(-s1,l,l)+2.0*UU(1,-1,m,m)*densav(s1,l,l)*densav(-s1,l,l)
         AA(s1,m,:) = AA(s1,m,:) + densav(-s1,m,m)*( Dmsig(-s1,l,:) + Dmsig(s1,l,:) )
        endif
       enddo
        AA(s1,m,:) =-AA(s1,m,:) + matsubara(:)*imi+mmu-Ei(s1,m,m)-ueff(s1,m)- &
      ...
      do m=1,Nc
        AA(s1,m,:)=0.
        AA(s1,m,:) = matsubara(:)*imi+mmu-Ei(s1,m,m)-ueff(s1,m)- &
                    & Dmsig(s1,m,:)-Dmasig(s1,m,:)-DmasigTild(s1,m,:)
        AA(s1,m,:) = ueff(s1,m) / AA(s1,m,:)
      enddo
      enddo
      ...
      do l=1,Nc
       ...
       do lp=1,Nc 
        if(lp/=l)then
         BB(s1,m,l,:) = BB(s1,m,l,:) +    ( UU(s1,s1,lp,m)*densav(s1,lp,lp)+ &
                                    &       UU(s1,-s1,lp,m)*densav(-s1,lp,lp)+ & 
                                    &   2.0*UU(1,-1,lp,lp)*densav(s1,lp,lp)*densav(-s1,lp,lp) )
         BB(s1,m,l,:) = BB(s1,m,l,:) +  ( densav(-s1,lp,lp) + densav(s1,lp,lp) )*( Dmsig(s1,m,:) ) 
        endif
       enddo
        BB(s1,m,l,:) =       - BB(s1,m,l,:) + matsubara(:)*imi + mmu - Ei(s1,m,m) - UUeff(s1,s1,l,m)   &
                                & - ( UU(1,-1,m,m)*densav(-s1,m,m) + UU(-s1,s1,l,m)*densav(-s1,l,l) + &
                                &     UU(1,-1,m,m)*densav(s1,m,m)*densav(-s1,m,m) +  & 
                                &     UU(1,-1,l,l)*densav(s1,l,l)*densav(-s1,l,l)  ) &
                                & -  (Dmsig(s1,m,:)-Dmbsig(s1,l,m,:)-DmbsigTild(s1,l,m,:))   &
                                & -  (densav(-s1,m,m) + densav(-s1,l,l)) * ( Dmsig(s1,m,:) ) 
      ...
      do l=1,Nc
       ...
       do lp=1,Nc
        if(lp/=l)then
          CC(s1,m,l,:) = CC(s1,m,l,:) + ( UU(s1,s1,lp,m)*densav(s1,lp,lp)+UU(s1,-s1,lp,m)*densav(-s1,lp,lp)+2.0*UU(1,-1,lp,lp)*densav(s1,lp,lp)*densav(-s1,lp,lp))
          CC(s1,m,l,:) = CC(s1,m,l,:) + ( densav(-s1,lp,lp) + densav(s1,lp,lp) )*( Dmsig(s1,m,:) ) 
        endif
       enddo
        CC(s1,m,l,:) =-CC(s1,m,l,:) + matsubara(:)*imi+mmu-Ei(s1,m,m)-UUeff(-s1,s1,l,m)   &
                                & - ( UU(1,-1,m,m)*densav(-s1,m,m) + UU(s1,s1,l,m)*densav(s1,l,l) + UU(1,-1,m,m)*densav(s1,m,m)*densav(-s1,m,m) + UU(1,-1,l,l)*densav(s1,l,l)*densav(-s1,l,l)  ) &
                                & - (Dmsig(s1,m,:)-Dmcsig(s1,l,m,:)-DmcsigTild(s1,l,m,:)) - ( densav(-s1,m,m) + densav(s1,l,l) )*( Dmsig(s1,m,:) ) 
      ...
      do s1=-1,1,2
       ...
       do m=1,Nc
        do l=1,Nc
         if(l/=m)then
          Gdyn_m(s1,m,m,:) =  Gdyn_m(s1,m,m,:) + BB(s1,m,l,:)*(Dmsig(s1,m,:)*I1b(s1,m,l,:)+I2b(s1,m,l,:))+CC(s1,m,l,:)*(Dmsig(s1,m,:)*I1c(s1,m,l,:)+I2c(s1,m,l,:))
         endif
        enddo
        Gdyn_m(s1,m,m,:) =  - Gdyn_m(s1,m,m,:) + matsubara(:)*imi +mmu - Ei(s1,m,m) - Dmsig(s1,m,:) &
                          & - AA(s1,m,:)*(Dmsig(s1,m,:)*I1a(s1,m,:)+I2a(s1,m,:)) 

        tempvec=0.0
        do l=1,Nc
         if(l/=m)then
          tempvec(:) = tempvec(:) + BB(s1,m,l,:) * (densav( s1,l,l)+I1b(s1,m,l,:))  &
                                & + CC(s1,m,l,:) * (densav(-s1,l,l)+I1c(s1,m,l,:))
         endif
        enddo 
...
!#####################################
   ...
   if(allocated(Dmasig)) deallocate(Dmasig,Dmbsig,Dmcsig,DmasigTild,DmbsigTild,DmcsigTild,Dmsig,AA,BB,CC,tempvec)
   if(allocated(I1a))    deallocate(I1a,I1b,I2a,I2b,I1c,I2c)

   allocate(I1a(-1:1,Nc,nw_r),I1b(-1:1,Nc,Nc,nw_r),I2a(-1:1,Nc,nw_r),I2b(-1:1,Nc,Nc,nw_r),I1c(-1:1,Nc,Nc,nw_r),I2c(-1:1,Nc,Nc,nw_r))
   allocate(Dmasig(-1:1,Nc,nw_r),Dmbsig(-1:1,Nc,Nc,nw_r))
   allocate(Dmcsig(-1:1,Nc,Nc,nw_r),DmasigTild(-1:1,Nc,nw_r),DmbsigTild(-1:1,Nc,Nc,nw_r),DmcsigTild(-1:1,Nc,Nc,nw_r))
   allocate(Dmsig(-1:1,Nc,nw_r))
   allocate(AA(-1:1,Nc,nw_r),BB(-1:1,Nc,Nc,nw_r),CC(-1:1,Nc,Nc,nw_r),tempvec(nw_r)) 
   ...
   write(*,*) 'FMOS REAL FREQUENCIES : ', frequ__(1:2)
     ...
     !----------------!
      ...
      do s1=-1,1,2
        ...
        do i=1,nw_r
          ...
          Gdyn_r(s1,1:Nc,1:Nc,i)=Gdyn_r(s1,1:Nc,1:Nc,i) - S_ff_r(s1,1:Nc,1:Nc,i)
     ...
     !-------------------------------------------------------------!
      ...
      do m=1,Nc
       ...
       do k=1,Nb
        Dmasig(s1,m,:) = Dmasig(s1,m,:) + conjg(Vb(-s1,m,k)) * Vb(-s1,m,k) / &
                   & ( frequ__(:)+mmu+Ei(-s1,m,m)-Ei(s1,m,m)-Eb(-s1,k,k)  )
       enddo
      enddo
      enddo
      ...
      do l=1,Nc
       ...
       do k=1,Nb
        Dmbsig(s1,l,m,:) = Dmbsig(s1,l,m,:) + conjg(Vb(s1,l,k)) * Vb(s1,l,k) / &
                   & ( frequ__(:)+mmu+Ei(s1,l,l)-Ei(s1,m,m)-Eb(s1,k,k)  )
       enddo
       enddo
      enddo
      enddo
      ...
      do m=1,Nc
       ...
       do k=1,Nb
        DmasigTild(s1,m,:) = DmasigTild(s1,m,:) + conjg(Vb(-s1,m,k)) * Vb(-s1,m,k) / &
                   & ( frequ__(:)+mmu + Eb(-s1,k,k) - Ei(-s1,m,m) - Ei(s1,m,m) - Umma(s1,m)   )
       enddo
      enddo
      enddo
      ...
      do l=1,Nc
       ...
       do k=1,Nb
        DmbsigTild(s1,l,m,:) = DmbsigTild(s1,l,m,:) + conjg(Vb(s1,l,k)) * Vb(s1,l,k) / &
                   & ( frequ__(:)+mmu + Eb(s1,k,k) - Ei(s1,l,l) - Ei(s1,m,m) - Ummb(s1,l,m)   )
       enddo
      enddo
      enddo
      enddo
      ...
      do l=1,Nc
       ...
       do k=1,Nb
        DmcsigTild(s1,l,m,:) = DmcsigTild(s1,l,m,:) + conjg(Vb(-s1,l,k)) * Vb(-s1,l,k) / &
                   & ( frequ__(:)+mmu + Eb(-s1,k,k) - Ei(-s1,l,l) - Ei(s1,m,m) - Ummc(s1,l,m)   )
       enddo
      enddo
      enddo
      enddo
    ...
    do l=1,Nc
     ...
     do kp=1,Nb
      I2b(s1,m,l,:) = I2b(s1,m,l,:) - conjg(Vb(s1,l,k)) * Vb(s1,l,k) * densav(s1,Nc+kp,Nc+k) / &
                 & ( frequ__(:)+mmu+Ei(s1,l,l)-Ei(s1,m,m)-Eb(s1,k,k)  )
      I2b(s1,m,l,:) = I2b(s1,m,l,:) - conjg(Vb(s1,l,k)) * Vb(s1,l,k) * densav(s1,Nc+k,Nc+kp) / &
                 & ( frequ__(:)+mmu+Eb(s1,k,k)-Ei(s1,m,m)-Ei(s1,l,l)-Ummb(s1,l,m)  )
     enddo
     enddo
    enddo
    enddo
   ...
   do s1=-1,1,2
    ...
    do l=1,Nc
     ...
     if(use_def_feng)then
      do k=1,Nb/Nc
        ...
        j=(l-1)*mstep
        I1c(s1,m,l,:) = I1c(s1,m,l,:) - Vbath(-s1,m,k) * densav(-s1,Nc+k+j,l) / &
                 & ( frequ__(:)+mmu+Eb(-s1,k+j,k+j)-Ei(-s1,l,l)-Ei(s1,m,m)-Ummc(s1,l,m)  )
      enddo
     endif
    ...
    if(fmos_fluc)then
      ...
      do m=1,Nc
       ...
       enddo
        AA(s1,m,:) =-AA(s1,m,:) + frequ__(:)+mmu-Ei(s1,m,m)-ueff(s1,m)- &
                                & Dmsig(s1,m,:)-Dmasig(s1,m,:)-DmasigTild(s1,m,:)
        AA(s1,m,:) = ueff(s1,m) / AA(s1,m,:)
      enddo
      enddo
      ...
      do l=1,Nc
       ...
       do lp=1,Nc 
        if(lp/=l)then
         BB(s1,m,l,:) = BB(s1,m,l,:) + ( UU(s1,s1,lp,m)*densav(s1,lp,lp)+UU(s1,-s1,lp,m)*densav(-s1,lp,lp)+2.0*UU(1,-1,lp,lp)*densav(s1,lp,lp)*densav(-s1,lp,lp) )
         BB(s1,m,l,:) = BB(s1,m,l,:) + ( densav(-s1,lp,lp) + densav(s1,lp,lp) )*( Dmsig(s1,m,:) )
        endif
       enddo
        BB(s1,m,l,:) =-BB(s1,m,l,:) + frequ__(:)+mmu-Ei(s1,m,m)-UUeff(s1,s1,l,m)   &
                                & - ( UU(1,-1,m,m)*densav(-s1,m,m) + UU(-s1,s1,l,m)*densav(-s1,l,l) + UU(1,-1,m,m)*densav(s1,m,m)*densav(-s1,m,m) + UU(1,-1,l,l)*densav(s1,l,l)*densav(-s1,l,l)  ) &
                                & - (Dmsig(s1,m,:)-Dmbsig(s1,l,m,:)-DmbsigTild(s1,l,m,:)) - ( densav(-s1,m,m) + densav(-s1,l,l) )*( Dmsig(s1,m,:) )
        BB(s1,m,l,:) = UUeff(s1,s1,l,m) / BB(s1,m,l,:)
       endif
      enddo
      enddo
      ...
      do l=1,Nc
       ...
       enddo
        CC(s1,m,l,:) =-CC(s1,m,l,:) + frequ__(:)+mmu-Ei(s1,m,m)-UUeff(-s1,s1,l,m)   &
                                & - ( UU(1,-1,m,m)*densav(-s1,m,m) + UU(s1,s1,l,m)*densav(s1,l,l) + UU(1,-1,m,m)*densav(s1,m,m)*densav(-s1,m,m) + UU(1,-1,l,l)*densav(s1,l,l)*densav(-s1,l,l)  ) &
                                & - (Dmsig(s1,m,:)-Dmcsig(s1,l,m,:)-DmcsigTild(s1,l,m,:)) - ( densav(-s1,m,m) + densav(s1,l,l) )*( Dmsig(s1,m,:) ) 
        CC(s1,m,l,:) = UUeff(-s1,s1,l,m) / CC(s1,m,l,:)
       endif
      ...
      do s1=-1,1,2
       ...
       do m=1,Nc
        do l=1,Nc
          if(l/=m)then
            Gdyn_r(s1,m,m,:) =  Gdyn_r(s1,m,m,:) + BB(s1,m,l,:)*(Dmsig(s1,m,:)*I1b(s1,m,l,:)+I2b(s1,m,l,:))+CC(s1,m,l,:)*(Dmsig(s1,m,:)*I1c(s1,m,l,:)+I2c(s1,m,l,:))
          endif
        enddo
        Gdyn_r(s1,m,m,:) =  - Gdyn_r(s1,m,m,:) + frequ__(:)+mmu-Ei(s1,m,m) - Dmsig(s1,m,:) - AA(s1,m,:)*(Dmsig(s1,m,:)*I1a(s1,m,:)+I2a(s1,m,:)) 
        tempvec=0.0
        do l=1,Nc
         if(l/=m)then
          tempvec(:) = tempvec(:) + BB(s1,m,l,:)*(densav( s1,l,l)+I1b(s1,m,l,:)) + &
        ...
        enddo
        Gdyn_r(s1,m,m,:) = 1.d0/Gdyn_r(s1,m,m,:)  * &
                    & ( 1.d0 + AA(s1,m,:)*(densav(-s1,m,m)+I1a(s1,m,:)) + tempvec(:)  ) 
       enddo
      enddo
...
implicit none
 ...
 logical          :: green
  ...
  call add_substract_tail(nw,Giom,iom,ahh,ddd,add=.false.,tail=tailmax)
