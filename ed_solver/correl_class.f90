...
contains
   ...
   subroutine new_correl_from_old(CORRELOUT, CORRELIN)
      ...
      CASE(BOSONIC, FERMIONIC)
         CALL new_correl_from_scratch_ifreq(CORRELOUT, CORRELIN%title, &
              ...
              IMASK = CORRELIN%MM%MASK%imat, AB = T)
   ...
   subroutine copy_correl(CORRELOUT, CORRELIN)
      ...
      use frequency_class, only: copy_frequency
   ...
   subroutine read_raw_correl(CORREL, UNIT, FILEIN)
      ...
      use frequency_class, only: delete_frequency, freq_type, read_raw_freq
      ...
      CASE(FERMIONIC, BOSONIC)
         CALL new_correl_from_scratch_ifreq(CORREL, title, N, FREQ%Nw, &
              FREQ%beta, STAT = stat, IMASK = MM%MASK%imat, AB = T)
      CASE(RETARDED, ADVANCED)
   ...
   subroutine dump_chi0(G, beta)
      ...
      do j = 1, size(G%vec, 1)
         ...
         DO iw = 1, Nw
            ...
            enddo
         ENDDO
         !!call plotarray( AIMAG(G%freq%vec(1:Nw/2)), real(chi0(1:Nw/2)),
         ! 'real_part_matsubara_Chi0' // toString(j) )
         !!call plotarray( AIMAG(G%freq%vec(1:Nw/2)), aimag(chi0(1:Nw/2)),
         ! 'im_part_matsubara_Chi0' // toString(j) )
      enddo
   ...
   subroutine write_correl(CORREL, FILEOUT)
      ...
      CASE(RETARDED, ADVANCED)
         ...
         !!call plotarray( DBLE(CORREL%freq%vec), aimag(CORREL%vec),
         ! 'im_part_retarded_' // TRIM(ADJUSTL(CORREL%title)))
      END SELECT
   ...
   contains
      ...
      subroutine dump_graphs()
         ...
         do j = 1, size(CORRELIN%vec, 1)
            ...
            ! real(CORRELIN%freq%vec), aimag(CORRELOUT%vec(j, :)), 'im transfo
            ! correl' // toString(j))
         enddo
      end subroutine
