...
contains
   ...
   subroutine four_leg_vertex_matrices_routine(AIM, GS)
      ...
      use eigen_sector_class,  only: eigensectorlist_type, partition
   ...
   SUBROUTINE four_leg_vertex_matrices(AIM, GS, Asector, applyA, cdagger)
      ...
      TYPE(eigen_type),       POINTER           :: eigen => NULL(), OPeigen => &
                                                   NULL()
      INTEGER                                   :: i1, j1, isec_back, ii, i, &
                                                   i_size, j_size
      INTEGER                                   :: ipm, jpm, kpm, iph, iorb, &
                                                   jorb, jjj1, jjj2, j
      LOGICAL                                   :: orb(1)
      INTEGER                                   :: isector, ieigen, uup, ddn, &
                                                   itot, i_, v(2), issz
      ...
      DO isector = 1, GS%nsector
         ...
         DO kpm = 1, 2
            ...
            if(kpm == 1.and.verbose) write(*, *) &
                 '-------------C^+----------------'
            if(kpm == 2.and.verbose) write(*, *) &
                 '--------------c-----------------'

            CALL delete_eigenlist(Apm_es(kpm)%lowest)
            if(verbose) write(*, *) 'starting loop over states in sector, there&
                 & are [x] states : ', GS%es(isector)%lowest%neigen

            DO ieigen = 1, GS%es(isector)%lowest%neigen
               CALL applyA(Apm_es(kpm), pm(kpm), orb, es, ieigen)
               if(verbose) write(*, *) 'applying A, neigen = ', &
                    Apm_es(kpm)%lowest%neigen
            ENDDO

            do ii = 1, GS%nsector
               if(GS%es(ii)%sector%updo%up%npart == &
                    Apm_es(kpm)%sector%updo%up%npart.and. &
                    GS%es(ii)%sector%updo%down%npart == &
                    Apm_es(kpm)%sector%updo%down%npart)then
                  exit
               endif
            enddo
            ...
            if(verbose) then
               write(*, *) 'applyA is in sector and max is         : ', &
                    ii, GS%nsector
               write(*, *) 'in target sector there are [x] states  : ', &
                    GS%es(ii)%lowest%neigen
               write(*, *) 'up, dn in original sector              : ', &
                    GS%es(isector)%sector%updo%up%npart, &
                    GS%es(isector)%sector%updo%down%npart
               write(*, *) 'up, dn in target sector                : ', &
                    GS%es(ii)%sector%updo%up%npart, &
                    GS%es(ii)%sector%updo%down%npart
               write(*, *) 'there are [x] collected states from A  : ', &
                    Apm_es(kpm)%lowest%neigen
               write(*, *) 'destroy or creates                     : ', &
                    kpm, pm(kpm)
               write(*, *) 'target/initial sector sizes            : ', &
                    i_size, j_size
            endif

            if(Apm_es(kpm)%lowest%neigen/=GS%es(isector)%lowest%neigen)then
               write(*, *) 'obtained c|i> and initial sector size : ', &
                    Apm_es(kpm)%lowest%neigen, GS%es(isector)%lowest%neigen
            ...
            if(kpm == 1)then
               ...
               do i = 1, i_size
                  do j = 1, j_size
                     cdagger(isector)%c_p(i, j) = scalprod( &
                          GS%es(ii)%lowest%eigen(i)%vec%rc, &
                          Apm_es(kpm)%lowest%eigen(j)%vec%rc)
                  enddo
               enddo
               if(verbose) write(*, *) 'maxval(cdagger) : ', &
                    maxval(abs(cdagger(isector)%c_p))
            else
               ...
               do i = 1, i_size
                  do j = 1, j_size
                     cdagger(isector)%c_m(i, j) = scalprod( &
                          GS%es(ii)%lowest%eigen(i)%vec%rc, &
                          Apm_es(kpm)%lowest%eigen(j)%vec%rc)
                  enddo
               enddo
               if(verbose) write(*, *) 'maxval(cdagger) : ', &
                    maxval(abs(cdagger(isector)%c_m))
            endif
         44 ENDDO
