...
!CW_MPI check how many frequ in omega_list
     ...
     j_=0
     do 
      read(4848, *,end=871) op,w1r,w2r,w3r
      j_=j_+1
      write(*,*) "in file op         = ", op
      write(*,*) "in file w1r, w2r, w3r = ", w1r, w2r, w3r
     enddo
   871 continue

     write(*,*) 'THERE ARE [x] FREQUENCIES IN FILE : ', j_
     write(*,*) 'allocating Chi_loc array'
     if(allocated(Chi_loc)) deallocate(Chi_loc)
     allocate(Chi_loc(j_),frequ_(j_,3),op_(j_))
     Chi_loc = 0.d0
     op_         = 0
     frequ_      = 0.d0
     rewind(4848)
     ...
     do i_=1,j_
       read(4848, *) op,w1r,w2r,w3r
       write(*,*) "op         = ", op
       write(*,*) "w1r, w2r, w3r = ", w1r, w2r, w3r
       op_(i_)      = op
...
!      write(*,*) 'pDNDN, pUPDN = ', pDNDN, pUPDN
      Chi_loc(i_) = Chi_loc(i_) + pDNDN + pUPDN
!
     enddo
!     close(5486)
...
!CW_MPI 
    ...
    call mpisum(Chi_loc) !collect array Chi_lattice

     write(*,*) 'Chi is : ' , Chi_loc
     ...
     if(rank==0)then
      ...
      open(unit=8080,file='Chi_loc_output')
      do i_=1,j_
       write(8080,*) Chi_loc(i_)
      enddo
      close(8080)
     endif
