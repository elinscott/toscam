...
contains
   ...
   subroutine Block_Lanczos_diagonalize(lowest)
      ...
      use eigen_class,         only: add_eigen, delete_eigen, eigen_type, &
           eigenlist_type, new_eigen
      use genvar,              only: all_log_unit, error, ierr, iproc, log_unit, &
           no_mpi, nproc, rank, size2
      ...
      IF(LFLAG < 0) then
         ...
         STOP "ERROR IN Block_Lanczos_diagonalize_real: BLZDRD WORKSPACE QUERY &
              &FAILED!"
      ENDIF
      ...
      DO
         ...
         IF(LFLAG < 0) STOP "ERROR IN Block_Lanczos_diagonalize_real: BLZDRD &
              &EXITED WITH LFLAG < 0!"
         ...
         CASE(1)    ! BLDRD REQUIRES V(:, 1:NVOPU) = H * U(:, 1:NVOPU)
            ...
            DO ivp = 1, NVOPU
               ...
               ! U(:, ivp)
               if(size2 > 1) then
                  CALL MPI_ALLGATHERV(U(:, ivp), local_chunk(iproc), &
                       MPI_DOUBLE_PRECISION, vec_in, local_chunk, &
                       local_istatemin-1, MPI_DOUBLE_PRECISION, &
                       MPI_COMM_WORLD, ierr)
               else
                  vec_in = U(:, ivp)
               endif
               ...
               ! BLZDRD REQUIRES LOCAL CHUNK ONLY => DISTRIBUTE vec_out OVER
               ! LOCAL V(:, ivp)
               V(:, ivp) = &
                    vec_out(local_istatemin(iproc):local_istatemax(iproc))
            ENDDO
         ...
         CASE(4)  ! BLDRD REQUIRES THE STARTING VECTORS IN V(:, 1:NVOPU)
            ...
            STOP "ERROR IN Block_Lanczos_diagonalize_real: BLZDRD EXITED WITH &
                 &UNEXPECTED LFLAG!"
      ...
      ! NOW WE HAVE THE DESIRED EIGENPAIRS THANKS TO BLZPACK IT JUST REMAINS
      ! TO STORE THEM IN EIGENLIST

      Nconverged = ISTOR(67) ! NUMBER OF CONVERGED EIGENPAIRS (MAY BE MORE
      ! THAN Neigen_ REQUIRED!)
      Niter      = ISTOR(19)  ! NUMBER OF BLOCK LANCZOS ITERATIONS PERFORMED

      write(*,*) '===================================================================='    
      write(*, *) ' Nconverged eigenpairs : ', rank, Nconverged, &
           VALP(1:Nconverged, 1)
      write(*, *) '  Niter                  : ', rank, Niter
      write(*,*) '===================================================================='    

      IF(Nconverged <= 0) then
         write(*, *) "ERROR IN Block_Lanczos_diagonalize_real: NO EIGENPAIR IS &
              &CONVERGED AFTER " // c2s(i2c(Niter)) // " ITERATIONS!"
         write(*, *) 'ISTOR: '
         write(*, *)  ISTOR
         stop
      endif
      ...
      DO ivp = 1, Nconverged
         if(abs(VALP(ivp, 1)-minv_) <= dEmax)then
            ...
            eigen%converged = .true.
            eigen%rank      = ivp
            if(size2 > 1) then
               CALL MPI_ALLGATHERV(VECP(:, ivp), local_chunk(iproc), &
                    MPI_DOUBLE_PRECISION, eigen%vec%rc, local_chunk, &
                    local_istatemin-1, MPI_DOUBLE_PRECISION, MPI_COMM_WORLD, &
                    ierr)
            else
      ...
      IF(Nconverged > Neigen_) CALL dump_message(TEXT = "# WARNING IN &
           &Block_Lanczos_diagonalize_real: " // c2s(i2c(Nconverged)) // " &
           &EIGENPAIRS CONVERGED > " // c2s(i2c(Neigen_)) // " REQUESTED")

      IF(Nconverged < Neigen_)THEN
         CALL dump_message(TEXT = "# WARNING IN &
              &Block_Lanczos_diagonalize_real: " // c2s(i2c(Neigen_)) // " &
              &EIGENPAIRS REQUESTED " // "BUT ONLY " // c2s(i2c(Nconverged)) &
              // " ARE CONVERGED AFTER" // c2s(i2c(Niter)) // " ITERATIONS")
         WRITE(log_unit, '(a, f12.10, a)') "# THE FOLLOWING EIGENVALUES &
              &(RESIDUALS) ARE DISCARDED [tolerance = ", tolerance, "] :"
         DO ivp = Nconverged + 1, Neigen_
            WRITE(log_unit, '(a, E20.12, a, E9.1, a)') "E" // c2s(i2c(ivp)) // &
                 " = ", VALP(ivp, 1), " (", VALP(ivp, 2), ")"
         ENDDO
      ENDIF
      ...
      CALL timer_fortran(start_diagH, "# DIAGONALIZATION OF " // &
           TRIM(ADJUSTL(title_H_())) // " TOOK " // c2s(i2c(Niter)) // " &
           &ITERATIONS AND ")
