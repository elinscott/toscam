...
AIM2%BATHiorb = (/AIM%BATHiorb(:,1),AIM%BATHiorb(:,2)/)
 ...
 AIM2%BATHiorb = AIM%BATHiorb(:,1)
    ...
    IF(AIM2%SUPER)THEN ! NAMBU 
      IF(PRESENT(SPIN)) STOP "ERROR IN new_AIM2: NO NEED TO SPECIFY SPIN DIRECTION!!"
    ...
    ELSE ! FIXED SPIN
      IF(.NOT.PRESENT(SPIN)) STOP "ERROR IN new_AIM2: SHOULD SPECIFY A SPIN DIRECTION!!"
      AIM2%spin = SPIN
      ! these tmp matrices enforce tabulation of sector up & down in the NAMBU basis 
    ...
    ! WARNING: HERE THE MASKS STAND FOR NON-ZERO MATRIX ELEMENT ... AVOIDS UNNECESSARY TABULATIONS
    
    AIM2%Ec%rc%MASK%mat = T
    WHERE(AIM2%Ec%rc%mat==zero)
      AIM2%Ec%rc%MASK%mat  = F
    END WHERE
    AIM2%Eb%rc%MASK%mat = T
    WHERE(AIM2%Eb%rc%mat==zero)
      AIM2%Eb%rc%MASK%mat  = F
    END WHERE
    AIM2%Vbc%rc%MASK%mat = T
    WHERE(AIM2%Vbc%rc%mat==zero)
      AIM2%Vbc%rc%MASK%mat = F
    END WHERE
