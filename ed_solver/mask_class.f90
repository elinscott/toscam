...
contains
   ...
   subroutine order_matrix_elements(tabelem, MASK)
      ...
      ENDIF
      !-------------------------------------------------------------------------------!
      ! REORDER MATRIX ELEMENTS FOR PRETTY PRINT: DIAGONAL FIRST + GATHER (i,
      ! j) (j, i) !
      !-------------------------------------------------------------------------------!
   ...
   subroutine new_mask_from_scratch(MASK, n1, n2, IMASK, SYMMETRY)
      ...
      use matrix,     only: new_diag, write_array
      ...
      IF(n1 /= n2.AND.(SYMMETRY == HERM.OR.SYMMETRY == SYM)) then
         write(*, *) "ERROR IN new_mask: SYMMETRY = " // TRIM(SYMMETRY) // " &
              &SPECIFIED BUT " // c2s(i2c(n1)) // "x" // c2s(i2c(n2)) // " &
              &MATRIX!"
         stop 'termine'
      endif
      ...
      ALLOCATE(MASK%imat(n1, n2))
      ALLOCATE( MASK%mat(n1, n2))
      ...
      IF(PRESENT(IMASK))THEN
         ...
         IF(MASK%SYMMETRY /= EMPTY)THEN
            DO i1 = 1, MASK%n1
               DO i2 = i1 + 1, MASK%n2
                  IF(MASK%imat(i1, i2)*MASK%imat(i2, i1) /= 0) MASK%imat(i2, &
                       i1) = 0
               ENDDO
            ENDDO
         ENDIF
      ...
      IF(MASK%SYMMETRY /= EMPTY)then
         if(size(MASK%imat(:, 1)) == size(MASK%imat(1, :)))then
            if(ANY((.NOT.is_diag).AND.TRANSPOSE(MASK%imat)*MASK%imat /= 0))THEN
               CALL dump_message(TEXT = "ERROR IN new_mask: SYMMETRY = " // &
                    TRIM(MASK%SYMMETRY) // " INCONSISTENT WITH")
   ...
   subroutine new_mask_from_old(MASKOUT, MASKIN)
      ...
      CALL new_mask_from_scratch(MASKOUT, MASKIN%n1, MASKIN%n2, SYMMETRY = &
           MASKIN%SYMMETRY)
   ...
   subroutine read_raw_mask(MASK, UNIT)
      ...
      READ(UNIT, *) IMASK
      CALL new_mask_from_scratch(MASK, n1, n2, IMASK = IMASK, SYMMETRY = &
           SYMMETRY)
   ...
   subroutine pad_imask(MASK)
      ...
      IF(MASK%n1 /= MASK%n2)THEN
         CALL dump_message(TEXT = "ERROR in symmetrize_imask: imask must be a &
              &SQUARE matrix!")
         ...
         DO i2 = i1 + 1, MASK%n2
            IF(MASK%imat(i1, i2)*MASK%imat(i2, i1) /= 0)THEN
               IF(MASK%imat(i1, i2) /= sign_cplx*MASK%imat(i2, i1))THEN
                  CALL dump_message(TEXT = "ERROR in symmetrize_imask: &
                       &INCONSISTENT INPUT IMASK!")
                  CALL write_mask(MASK)
                  STOP
               ENDIF
            ELSE IF(MASK%imat(i1, i2) /= 0.OR. MASK%imat(i2, i1) /= 0)THEN
               IF(MASK%imat(i1, i2) == 0) MASK%imat(i1, i2) = sign_cplx * &
                    MASK%imat(i2, i1)
               IF(MASK%imat(i2, i1) == 0) MASK%imat(i2, i1) = sign_cplx * &
                    MASK%imat(i1, i2)
            ENDIF
         ENDDO
      ENDDO
   ...
   subroutine mask2vec(MASK)
      ...
      ENDIF
      MASK%nind = COUNT(MASK%mat)
      IF(ASSOCIATED(MASK%ivec).AND.SIZE(MASK%ivec) /= MASK%nind) &
           DEALLOCATE(MASK%ivec, STAT = istati)
      IF(.NOT.ASSOCIATED(MASK%ivec).AND.MASK%nind /= 0) &
           ALLOCATE(MASK%ivec(MASK%nind))
   ...
   subroutine slice_mask(MASKOUT, MASKIN, rbounds, cbounds)
      ...
      IF(.NOT.ASSOCIATED(MASKIN%mat)) then
         ...
         STOP
      ENDIF
      IF(rbounds(1) < LBOUND(MASKIN%mat, 1).OR.rbounds(2) > UBOUND(MASKIN%mat, &
           1)) then
         write(*, *) "ERROR IN slice_mask: INCONSISTENT ROW    BOUNDS!"
         STOP
      ENDIF
      IF(cbounds(1) < LBOUND(MASKIN%mat, 2).OR.cbounds(2) > UBOUND(MASKIN%mat, &
           2)) then
      ...
      ENDIF
      SYMslice = ''
      IF(ALL(rbounds == cbounds)) SYMslice = MASKIN%SYMMETRY ! SLICE =
      ! DIAGONAL BLOCK
