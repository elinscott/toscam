 ...
 subroutine scan_indices
  if(allocated(indices_state_sector)) deallocate(indices_state_sector)
    allocate(indices_state_sector(GS%nsector*2*2*2*Neigen*green%N*green%N,7)); ktot=0
  ...
  integer :: i 
   ...
   !--------------------!
    ...
    DO ipm=1,2; DO jpm=1,2 
       ...
       IF(compute_dyn_correl.AND.(green%compute(ipm,jpm).OR.green%compute(3-ipm,3-jpm))) CALL vec2correl(green%correl(ipm,jpm))
    ENDDO; ENDDO
   ...
   !----------!
    ...
    CALL timer_fortran(compute_green_timer,"### COMPUTING "//TRIM(green%title)//" GREEN S FUNCTION TOOK")
  ...
  logical :: do_orb
          ...
          !------------------------------------------------------------------!
            ...
            DO i_=start+1,green%N**2,step
              v=k_to_ij(green%N,i_); iorb=v(1); jorb=v(2);
              ...
              if(.not.(FLAG_MPI_GREENS==2).or.(iorb==iiorb.and.jorb==jjorb))then
                ...
                iind = find_rank(MASK(ipm,jpm)%imat(iorb,jorb),MASK(ipm,jpm)%ivec)
                if(messages3) write(*,*) 'SYM COMBINATION',rank,iind
                CALL symmetric_combineAA(Apmsym,iorb,jorb,ipm,jpm,iph,Apm_es,isec_back,iisector,GS,issz)
                if(present(keldysh_level))then
                  Apmsym%vec%rc = Apm_es(1)%lowest%eigen(rank_eigen_in_list(jorb,Apm_es(1)%lowest))%vec%rc
                endif
                normvec=0.d0
                !-------------------------!
                ...
                IF(iph==1) THEN
                  ...
                  green%correlstat(ipm,jpm)%rc%vec(iind)=green%correlstat(ipm,jpm)%rc%vec(iind) + boltz*normvec**2
                ENDIF
                !-----------------------!
                ! DYNAMIC  CORRELATIONS !
                !-----------------------!
                if(messages3) write(*,*) 'COMPUTE DYN COR'   , rank
                if(messages3) write(*,*) 'compute_dyn_correl', compute_dyn_correl
                if(messages3) write(*,*) 'compute_ipm_jpm'   , green%compute(ipm,jpm)

                IF(compute_dyn_correl.AND.green%compute(ipm,jpm)) THEN
                   CALL reset_timer(compute_dyn_timer)
                   CALL compute_dynamic(iph,dyn,green%correl(ipm,jpm)%freq,Apmsym,green%correl(ipm,jpm)%stat,green%title,normvec,iisector,GS,keldysh_level)
                   if(present(GS_out))then
                      if(.not.present(keldysh_level))then
                        write(*,*) 'ERROR, some inputs for keldysh are missing'
                        stop
                      endif
                      if(size(GS_out%es(isector)%lowest%eigen(ieigen)%vec%rc)/=size(Apmsym%vec%rc))then
                        write(*,*) 'ERROR Keldysh, problem in copying the eigenvalue to new time frame'
                        write(*,*) 'Size of sectors do not match'
                        stop
                      endif
                      if(iorb/=jorb)then
                        write(*,*) 'ERROR KELDYSH, should only be diagional orbitals'
                        stop
                      endif
                      GS_out%es(isector)%lowest%eigen(ieigen)%vec%rc=Apmsym%vec%rc
                   endif
                   green%correl(ipm,jpm)%vec(iind,:) = green%correl(ipm,jpm)%vec(iind,:) + boltz * dyn
                   CALL timer_fortran(compute_dyn_timer,"# COMPUTING MATRIX ELEMENT "//pm(iph)//c2s(i2c(iind))//"/"//c2s(i2c(MASK(ipm,jpm)%nind))//" TOOK ")
                ENDIF
  ...
  implicit none
       ...
       !------------------------------------------------------!
           ...
           DO kpm=1,2 
             ...
             CALL timer_fortran(applyA_timer,"# APPLYING A"//pm(kpm)//" ON EIGENSTATE TOOK")

         !--------------------------------------------------------------------------------!
         ! IF SECTOR IS STABLE UNDER OPERATOR, WE ALSO COMPUTE THE MEAN VALUE OF OPERATOR !
         !--------------------------------------------------------------------------------!
              ...
              IF(equal_sector(es%sector,Apm_es(kpm)%sector))THEN
                ...
                DO iorb=1,green%N; 
                 IF(ORBMASKvec(ipm,jpm,iorb))THEN
                   ...
                   green%Amean(iorb,kpm) =  green%Amean(iorb,kpm) + &
 ...
 !--------------------------!
  ...
  subroutine init_iph
        ...
        IF(green%compute(ipm,jpm))THEN
            SELECT CASE(iph)
            CASE(1)
              if(associated(Apm_es(jpm)%sector%sz)) isec_back=Apm_es(jpm)%sector%sz%dimen
            CASE(2)
              if(associated(Apm_es(3-jpm)%sector%sz)) isec_back=Apm_es(3-jpm)%sector%sz%dimen
            END SELECT
        ENDIF

        if(es%lowest%neigen==0) stop 'ERROR in green_computeAA : es%lowest%neigen=0'
  ...
  subroutine init_sector
     ...
     write(log_unit,*) ' ...... eigenvector  ...... : ', (/( maxval(abs( es%lowest%eigen(ii)%vec%rc )), ii=1,es%lowest%neigen )/)
      ...
      if(associated(GS%es(isector)%lowest%eigen)) then
       if(GS%es(isector)%lowest%neigen==0) stop 'ERROR in green_computeAA : GS%es%lowest%neigen=0'
      endif
      ...
      DO iph=1,2
        ...
        if(associated(es%sector%updo))then
         write(log_unit,*) '-------------------------------------------------------'
         write(log_unit,*) 'chunk up',   iph,   es%sector%updo%up%chunk   , es%sector%updo%up%title
         write(log_unit,*) 'chunk dn',   iph,   es%sector%updo%down%chunk , es%sector%updo%down%title
         if(associated(Apm_es(iph)%sector%updo))then
           write(log_unit,*) 'chunk up A', iph,   Apm_es(iph)%sector%updo%up%chunk,   Apm_es(iph)%sector%updo%up%title
           write(log_unit,*) 'chunk dn A', iph,   Apm_es(iph)%sector%updo%down%chunk, Apm_es(iph)%sector%updo%down%title
         endif
         write(log_unit,*) '-------------------------------------------------------'
        endif
 ...
 !--------------------------!
   ...
   integer :: ika1,ikb1
    ...
    CALL dump_message(TEXT="### START COMPUTING AA "//TRIM(green%title)//" GREEN S FUNCTION")
    ...
    DO ipm=1,2; DO jpm=1,2
      ...
      DO iorb=1,green%N
       IF(ANY(MASK(ipm,jpm)%mat(iorb,:)).OR.ANY(MASK(ipm,jpm)%mat(:,iorb))) ORBMASKvec(ipm,jpm,iorb) = T
      ENDDO
    ENDDO; ENDDO

    ! SET ONLY THOSE MATRIX ELEMENTS THAT WE ARE GOING TO COMPUTE TO ZERO
    DO ipm=1,2; DO jpm=1,2
      DO iorb=1,green%N; DO jorb=1,green%N; IF(MASK(ipm,jpm)%mat(iorb,jorb))THEN
      iind = find_rank(MASK(ipm,jpm)%imat(iorb,jorb),MASK(ipm,jpm)%ivec)
      if(iind==0)then
       write(*,*) 'ERROR index is zero! not supposed to happen'
       write(*,*) 'MASK OF THE GF (in green_moduleAA) : '
       do ika1=1,green%N;do ikb1=1,green%N
          write(*,*) 'MASK % IMAT (orb1,orb2) : ',ika1,ikb1,MASK(ipm,jpm)%imat(ika1,ikb1)
       enddo;enddo
       write(*,*) 'FOR IPM JPM : ', ipm,jpm
       write(*,*) 'MASK vec is : ', MASK(ipm,jpm)%ivec 
       write(*,*) 'GF vec is   : ', green%correlstat(ipm,jpm)%rc%MASK%ivec
       ...
       write(*,*) 'GREEN FUNCTION - CORREL - STATIC PART MASK : '
       do ika1=1,green%N;do ikb1=1,green%N
        write(*,*) ' GREEN % IMAT (orb1,orb2) ',ika1,ikb1,green%correlstat(ipm,jpm)%rc%MASK%imat(ika1,ikb1)
       enddo;enddo;
       stop
      endif 
      ! EQUAL-TIME
      green%correlstat(ipm,jpm)%rc%vec(iind) = zero
      ! DYNAMIC
      IF(compute_dyn_correl.AND.green%compute(ipm,jpm)) green%correl(ipm,jpm)%vec(iind,:) = zero
      ENDIF; ENDDO; ENDDO
    ENDDO; ENDDO

    ! MEAN
    DO iorb=1,green%N; IF(ANY(ORBMASKvec(:,:,iorb)))THEN
      green%Amean(iorb,:) = zero
    ENDIF; ENDDO
    ...
    write(log_unit,*) '-------------------' 
    write(log_unit,*) 'COMPUTING GREEN FUNCTION MASK (IPM,JPM) (hole-particle channels)'
