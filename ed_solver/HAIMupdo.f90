MODULE HAIMupdo_class
   ...
   INTEGER                  , PRIVATE :: iupmin, iupmax, idomin, idomax, &
...
contains
   ...
   subroutine new_HAIMupdo(AIM, sector_in)
      ...
      use genvar,                  only: iproc, log_unit, messages, rank
      use fermion_sector2_class,   only: fermion_sector2_type
      use matrix,                  only: new_diag
      use aim_class,               only: aim_type
      use haim2_class,             only: AIM2do, AIM2up, new_haim2, tab_haim2
      ...
      if(messages)then
         write(log_unit,*) ' ============================================================ ' 
         write(log_unit, *) ' on cpu ', iproc, ' chunk of states is : ', &
              iupmin, iupmax, idomin, idomax
   ...
   subroutine delete_HAIMupdo()
      ...
      use masked_matrix_class_mod, only: delete_masked_real_matrix
   ...
   subroutine HAIMupdo_multc(vec_out, vec_in)
      ...
      use globalvar_ed_solver,   only: do_quench, flag_mpi_greens, &
      ...
      INTEGER                   :: up_in_, do_in_, i1, i2, do_out_, up_out_, &
                                   fermion_sign_
      integer                   :: fermion_signb_, site3, site4, n3, n4, is1, &
                                   is2, up_outb_, do_outb_
      logical                   :: bn3(2), bn4(2)
      integer                   :: up_outbb_, do_outbb_, up_outbbb_, &
                                   do_outbbb_, fermion_signbb_, &
                                   fermion_signbbb_
      integer                   :: IMPstate, BATHup, BATHdo, iupimp, idoimp, &
                                   iupimp_out, idoimp_out, AIMup, AIMdo
      logical                   :: emptychunks
      ...
      if(OPEN_MP)then
         if(verboseall) write(*, *) 'ALLOCATING IMIN/IMAX WITH SIZE - RANK : &
              &', MAXT, RANK
      ...
      if(HILBERT_SPACE_SPLITED_AMONG_NODES .and. USE_CC) then
         if(.not.emptychunks) then
            vec_out( 1 + sum(sector%chunk(1:iproc-1)) : &
                 sum(sector%chunk(1:iproc)) ) = 0.d0
         else
            vec_out = 0.d0
         endif
      else
      ...
      go_for_omp = .false.
      if(OPEN_MP) go_for_omp = minval(imax_) > 0 .and. minval(sector%chunk) > &
           0. .and. minval(imin_) > 0

      !$OMP PARALLEL IF(go_for_omp) PRIVATE( IMPstate, BATHup, BATHdo, iupimp, &
      !$OMP    idoimp, iupimp_out, idoimp_out, AIMup, AIMdo, up_outbb_, do_outbb_, &
      !$OMP    up_outbbb_, do_outbbb_, fermion_signbb_, fermion_signbbb_, &
      !$OMP    fermion_signb_, site3, site4, n3, n4, is1, is2, up_outb_, do_outb_, &
      !$OMP    bn3, bn4, n1, n2, iupb, idob, iupmin0, iup, iupmin, iupmax, ido, &
      !$OMP    istate, TID, site1, bn, bn1, bn2, site2, iuploc, idoloc, istatemin, &
      !$OMP    istatemax, nhoffdiagup, nhoffdiagdo, noff, irank, hoffdiagup, &
      !$OMP    choffdiagup, hoffdiagdo, choffdiagdo, jdo, jup, jstate, up_in_, &
      !$OMP    do_in_, i1, i2, jj, do_out_, up_out_, fermion_sign_, istate_, jstate_) &
      !$OMP    SHARED(vec_out, vec_in, sec, diagup, diagdo)
      ...
      if(do_quench /= 2)then
         DO iup = iupmin, iupmax
            DO site1 = 1, Nc
               IF(UMASK(site1, site1) .AND. BTEST(sector%up%state(iup), &
                    IMPiorbup(site1)-1))THEN
                  DO ido = idomin, idomax
                     IF(BTEST(sector%down%state(ido), IMPiorbdo(site1)-1))THEN
                        istate          = iup + (ido-1) * sec
                        vec_out(istate) = vec_out(istate) + U(site1, site1) * &
                             vec_in(istate)
                     ENDIF
                  ENDDO
               ENDIF
            ...
            DO site1 = 1, Nc
               IF(UMASK(site1, site1) .AND. BTEST(sector%up%state(iup), &
                  ...
                  DO ido = idomin, idomax
                     IF(BTEST(sector%down%state(ido), IMPiorbdo(site1)-1))THEN
                        istate          = iup + (ido-1) * sec
                        vec_out(istate) = vec_out(istate) + quench_U * &
                             vec_in(istate)
                     ENDIF
                  ENDDO
               ENDIF
      ...
      IF(offdiag_coulomb)THEN
         DO iup = iupmin, iupmax
            DO ido = idomin, idomax
               DO site1 = 1, Nc-1
                  bn1 = (/BTEST(sector%up%state(iup), IMPiorbup(site1)-1), &
                       BTEST(sector%down%state(ido), IMPiorbdo(site1)-1)/)
                  n1  = COUNT(bn1)
                  IF(n1 /= 0)THEN
                     DO site2 = site1 + 1, Nc
                        IF(UMASK(site1, site2))THEN
                           bn2 = (/BTEST(sector%up%state(iup), &
                                IMPiorbup(site2)-1), &
                                BTEST(sector%down%state(ido), &
                                IMPiorbdo(site2)-1)/)
                           n2  = COUNT(bn2)
                           IF(n2 /= 0)THEN
                              istate = iup + (ido-1) * sec
                              vec_out(istate) = vec_out(istate) + U(site1, &
                                   site2) * n1 * n2 * vec_in(istate)
      ...
      DO iup = iupmin, iupmax
         ...
         DO ido = idomin, idomax
            idoloc          =  ido - idomin + 1
            istate          =  iup + (ido-1) * sec
            vec_out(istate) = vec_out(istate) + ( diagup(iuploc) + &
                 diagdo(idoloc) ) * vec_in(istate)
         ENDDO
      ENDDO
      ...
      DO iup = iupmin, iupmax
         ...
         DO irank = 1, noff
            ...
            DO istate = istatemin, istatemax, stridedo
               if(.not.USE_CC) vec_out(jstate) = vec_out(jstate) + &
                    hoffdiagup * vec_in(istate)
               vec_out(istate) = vec_out(istate) + choffdiagup * &
                    vec_in(jstate)
      ...
      DO ido = idomin, idomax ! span the local chunk of the ndo-sector
         ...
         DO irank = 1, noff
            ...
            DO istate = istatemin, istatemax, strideup
               if(.not.USE_CC) vec_out(jstate) = vec_out(jstate) + &
                    hoffdiagdo * vec_in(istate)
               vec_out(istate) = vec_out(istate) + choffdiagdo * &
                    vec_in(jstate)
      ...
      if(flag_slater_int .and. .not.use_precomputed_slater_matrix)then
         DO iup = iupmin, iupmax
            DO ido = idomin, idomax
               ...
               DO site4 = 1, Nc
                  bn4 = (/BTEST(up_in_, IMPiorbup(site4)-1), BTEST(do_in_, &
                       IMPiorbdo(site4)-1)/)
                  do is2 = 1, 2
                     ...
                     if(is2 == 1)then
                        ...
                        i2 = IMPiorbup(site4)
                        DO jj = 1, i2-1
                           IF(BTEST(up_in_, jj-1)) fermion_sign_ = &
                                -fermion_sign_
                        ENDDO
                     else
                        ...
                        i2 = IMPiorbdo(site4)
                        DO jj = 1, i2-1
                           IF(BTEST(do_in_, jj-1)) fermion_sign_ = &
                                -fermion_sign_
                        ENDDO
                     endif

                     DO site3 = 1, Nc
                        bn3 = (/BTEST(up_out_, IMPiorbup(site3)-1), &
                             BTEST(do_out_, IMPiorbdo(site3)-1)/)
                        do is1 = 1, 2
                           ...
                           DO site2 = 1, Nc
                              bn2 = (/BTEST(up_outb_, IMPiorbup(site2)-1), &
                                   BTEST(do_outb_, IMPiorbdo(site2)-1)/)
                              if(bn2(is1)) cycle
                              if(is1 == 1)then
                                 up_outbb_ = IBSET(up_outb_, &
                                      IMPiorbup(site2)-1)
                                 do_outbb_ = do_outb_
                                 ...
                                 DO jj = i1 + 1, i2-1
                                    IF(BTEST(up_outb_, jj-1)) &
                                         fermion_signbb_ = -fermion_signbb_
                                 ENDDO
                              else
                                 up_outbb_ = up_outb_
                                 do_outbb_ = IBSET(do_outb_, &
                                      IMPiorbdo(site2)-1)
                                 fermion_signbb_ = fermion_sign_
                                 i1 = min(IMPiorbdo(site2), IMPiorbdo(site3))
                                 i2 = max(IMPiorbdo(site2), IMPiorbdo(site3))
                                 DO jj = i1 + 1, i2-1
                                    IF(BTEST(do_outb_, jj-1)) &
                                         fermion_signbb_ = -fermion_signbb_
                                 ENDDO
                              endif
                              ...
                              DO site1 = 1, Nc
                                 bn1 = (/BTEST(up_outbb_, &
                                      IMPiorbup(site1)-1), BTEST(do_outbb_, &
                                      IMPiorbdo(site1)-1)/)
                                 if(bn1(is2)) cycle
                                 if(is2 == 1)then
                                    up_outbbb_ = IBSET(up_outbb_, &
                                         IMPiorbup(site1)-1)
                                    do_outbbb_ = do_outbb_
                                    fermion_signbbb_ = fermion_signbb_
                                    i2 = IMPiorbup(site1)
                                    DO jj = 1, i2-1
                                       IF(BTEST(up_outbb_, jj-1)) &
                                            fermion_signbbb_ = &
                                            -fermion_signbbb_
                                    ENDDO
                                 else
                                    up_outbbb_ = up_outbb_
                                    do_outbbb_ = IBSET(do_outbb_, &
                                         IMPiorbdo(site1)-1)
                                    fermion_signbbb_ = fermion_signbb_
                                    i2 = IMPiorbdo(site1)
                                    DO jj = 1, i2-1
                                       IF(BTEST(do_outbb_, jj-1)) &
                                            fermion_signbbb_ = &
                                            -fermion_signbbb_
                                    ENDDO
                                 endif

                                 jstate_ = sector%up%rank(up_outbbb_) + &
                                      (sector%down%rank(do_outbbb_)-1)*sec
                                 vec_out(istate_) = vec_out(istate_) + &
                                      vec_in(jstate_)*Slater_Coulomb_c(site1, &
                                      site2, site3, &
                                      site4)*dble(fermion_signbbb_)
         ...
         DO iup = iupmin, iupmax
            DO ido = idomin, idomax
               ...
               do site1 = 1, UCC(iupimp, idoimp, 1, 3)
                  ...
                  jstate_ = sector%up%rank(AIMup) + &
                       (sector%down%rank(AIMdo)-1)*sec
                  vec_out(istate_) = vec_out(istate_) + &
                       vec_in(jstate_)*UCCc(iupimp, idoimp, site1)
               enddo
            ENDDO
         ENDDO
      endif
                        ...
                        if(UMASK(site1, site2))then
                           ...
                           IF(n2 == 1)THEN
                              ...
                              if(.not.Jhund_Slater_type)then
                                 if((bn1(1) .and. bn2(1)) .or. (bn1(2) &
                                      .and. bn2(2)))then !up-dn
                                    !Si^z Sj^z : up up
                                    vec_out(istate_) = vec_out(istate_) - &
                                         0.50d0 * JJmatrix(site1, site2) * &
                                         vec_in(istate_)
                                 else
                                    vec_out(istate_) = vec_out(istate_) + &
                                         0.50d0 * JJmatrix(site1, site2) * &
      ...
      if(HILBERT_SPACE_SPLITED_AMONG_NODES) then
         ...
         if(USE_CC .and. .not.emptychunks)then
            ...
            if(sum(sector%chunk) /= dimen)then
               write(*, *) 'ERROR MPI ED : sum of chuncks different from &
                    &dimension of sector'
...
#endif
            call mpibcast(vec_out, sector%chunk, &
                 [(sum(sector%chunk(1:j-1)), j = 1, size2)] )
         else
   ...
   subroutine HAIMupdo_multr(vec_out, vec_in)
      ...
      integer                 :: up_outbb_, do_outbb_, up_outbbb_, do_outbbb_, &
                                 fermion_signbb_, fermion_signbbb_
      integer                 :: IMPstate, BATHup, BATHdo, iupimp, idoimp, &
                                 iupimp_out, idoimp_out, AIMup, AIMdo
      logical                 :: emptychunks
      ...
      DO iup = iupmin, iupmax
         ...
         DO irank = 1, noff
            ...
            DO istate = istatemin, istatemax, stridedo
               if(.not.USE_CC) vec_out(jstate) = vec_out(jstate) + &
                    hoffdiagup * vec_in(istate)
               vec_out(istate) = vec_out(istate) + hoffdiagup * &
                    vec_in(jstate)
               jstate          = jstate          + stridedo
            ENDDO
         ENDDO
      ...
      DO ido = idomin, idomax
         ...
         DO irank = 1, noff
            ...
            DO istate = istatemin, istatemax, strideup
               if(.not.USE_CC) vec_out(jstate) = vec_out(jstate) + &
                    hoffdiagdo * vec_in(istate)
               vec_out(istate) = vec_out(istate) + hoffdiagdo * &
                    vec_in(jstate)
               jstate = jstate + strideup
            ENDDO
         ENDDO
                           ...
                           DO site2 = 1, Nc
                              ...
                              DO site1 = 1, Nc
                                 ...
                                 jstate_ = sector%up%rank(up_outbbb_) + &
                                      (sector%down%rank(do_outbbb_)-1)*sec
                                 vec_out(istate_) = vec_out(istate_) + &
                                      vec_in(jstate_)*Slater_Coulomb_r(site1, &
                                      site2, site3, &
                                      site4)*dble(fermion_signbbb_)
            ...
            DO ido = idomin, idomax
               ...
               do site1 = 1, UCC(iupimp, idoimp, 1, 3)
                  ...
                  jstate_ = sector%up%rank(AIMup) + &
                       (sector%down%rank(AIMdo)-1)*sec
                  vec_out(istate_) = vec_out(istate_) + &
                       vec_in(jstate_)*UCCr(iupimp, idoimp, site1)
               enddo
            ENDDO
         ENDDO
      endif
                        ...
                        if(UMASK(site1, site2))then
                           ...
                           IF(n2 == 1)THEN
                              if(bn1(1) .and. bn2(2))then
                                 ...
                                 idob = sector%down%rank(do_out_)
                                 jstate_ = iupb + (idob-1)*sec
                                 !vec_out(jstate_) = vec_out(jstate_) +
                                 ! vec_in(istate_)*JJmatrix(site1,
                                 ! site2)*dble(fermion_sign_)
                                 vec_out(istate_) = vec_out(istate_) + &
                                      vec_in(jstate_)*JJmatrix(site1, &
                                      site2)*dble(fermion_sign_) !C.C.
                              endif
   ...
   contains
      ...
      subroutine check_openmp()
         ...
         use globalvar_ed_solver, only: open_mp
         ...
         if(iup > size(sector%up%state))then
            ...
            write(*, *) 'size of hilbert space : ', size(sector%up%state), &
                 sector%up%dimen
   ...
   subroutine HAIMupdo_multr_split(vec_out, vec_in)
      ...
      INTEGER    :: istateloc, istateminloc, istatemaxloc
      INTEGER    :: iup, ido, jup, jdo, iuploc, idoloc, irank
      INTEGER    :: noff, I, sec, imin_(MAXT), imax_(MAXT), TID, iupmin0, &
                    min_, max_
      INTEGER(8) :: nhoffdiagup, nhoffdiagdo
      LOGICAL    :: bn(2), bn1(2), bn2(2)
      ...
      if(iupmin /= 1) stop 'error use transpose trick for nup ndn basis, the &
           &striping of the up spin should be 1'
      ...
      IF(offdiag_coulomb)THEN
         DO iup = 1, iupmax
            DO ido = idomin, idomax
               DO site1 = 1, Nc-1
                  bn = (/BTEST(sector%up%state(iup), IMPiorbup(site1)-1), &
                       BTEST(sector%down%state(ido), IMPiorbdo(site1)-1)/)
                  n1 = COUNT(bn)
                  IF(n1 /= 0)THEN
                     DO site2 = site1 + 1, Nc
                        IF(UMASK(site1, site2))THEN
                           bn = (/BTEST(sector%up%state(iup), &
                                IMPiorbup(site2)-1), &
                                BTEST(sector%down%state(ido), &
                                IMPiorbdo(site2)-1)/)
                           n2 = COUNT(bn)
      ...
      DO iup = 1, iupmax
         ...
         DO ido = idomin, idomax
            istate          =  iup + (ido-idomin) * iupmax
            vec_out(istate) = vec_out(istate) + ( diagup(iup) + diagdo(ido) ) &
                 * vec_in(istate)
         ENDDO
      ENDDO
