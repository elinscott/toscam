...
complex(8),allocatable :: frequ__(:),Tail(:,:,:),matsubara(:),gimp(:,:,:),T1(:,:),T2(:,:)
...
real(8)                :: mmu_
     ...
     allocate(Tail(nmoments,size(Eimp,1),size(Eimp,2)),T1(2*Nc,2*Nc),T2(2*Nc,2*Nc))
    ...
    !==========================================================================!
     ...
     do imu=1,nmu_adapt
        ...
        enddo
        call gf_HI_fullU( GF= g_out , Tail=Tail, e0f=Eimp, Ur = Ur, umn = umn , ujmn=ujmn , &
                        & zmsb=matsubara,nlm=Nc,Iwmax=nw_m,nmom=2,                          &
                        & ns=2,atocc=nout,atmag=mout,temp=1.d0/beta_ED,verbosity=0 )
        call sigma_atomic_fullU(GF = g_out,Sigma_mat=self_out,e0f=Eimp,zmsb=matsubara,nlm=Nc,Iwmax=nw_m,ns=2)
        do i=1,2*Nc
         Eimp(i,i)=Eimp(i,i)+mmu_+mu_cor(imu)
        enddo
        gimp=0.0
        ...
        do i=1,size(gimp,1)
         ...
         !rdens(i) =  2.0*sum( real ( g_out(i,i,1:nw_m-10) ) )/beta_ED + 0.5d0 !BUG TO TEST
        enddo

        denshub(imu)=sum(rdens)
        write(*,'(a,100f10.3)') ' occ. HUB           : ', sum(nout)
        write(*,'(a,100f10.3)') ' occ. atom / target : ', denshub(imu),target
    ...
    !==========================================================================!
     ...
     if(nmu_adapt>1)then
       ...
       write(*,*) '=========== DIFF FROM TARGET ============', abs( denshub(mu_min) - target )
       do i=1,2*Nc
    ...
    !==========================================================================!
      ...
      do i=1,2*Nc
        Eimp(i,i)=Eimp(i,i)-mmu_
      enddo
      call gf_HI_fullU( GF= gw , Tail=Tail, e0f=Eimp, Ur = Ur, umn = umn , ujmn=ujmn , &
                      & zmsb=frequ__,nlm=Nc,Iwmax=nw_r,nmom=2,                         &
                      & ns=2,atocc=nout,atmag=mout,temp=1.d0/beta_ED,verbosity=0)
      call sigma_atomic_fullU(GF = gw,Sigma_mat=sigw,e0f=Eimp,zmsb=frequ__,nlm=Nc,Iwmax=nw_r,ns=2)
      do i=1,2*Nc
        Eimp(i,i)=Eimp(i,i)+mmu_
      enddo
